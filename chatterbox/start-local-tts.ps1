...
$listener = Get-NetTCPConnection -LocalAddress '127.0.0.1' -LocalPort 8880 -State Listen -ErrorAction SilentlyContinue
if ($listener) {
...
$process = Start-Process `
    -FilePath $python `
    -ArgumentList $server, '--reference-audio', $ReferenceAudio, '--exaggeration', $Exaggeration, '--cfg-weight', $CfgWeight, '--temperature', $Temperature, '--cfm-steps', $CfmSteps, '--host', '127.0.0.1', '--port', '8880' `
