...
def resolve_voice(voice_name: str) -> dict:
    ...
    canonical_name = next(
        (name for name in voices if name.casefold() == voice_name.casefold()),
        None,
    )
    if canonical_name:
        return voices[canonical_name]
...
async def list_voices():
    return {
        "object": "list",
        "data": [{"id": name, "name": name} for name in voices],
    }
...
async def create_speech(req: SpeechRequest):
    ...
    # AIRI consumes the response as one ArrayBuffer. Generate a complete audio
    # file with truthful lengths instead of an open-ended streaming WAV header.
    loop = asyncio.get_event_loop()
        ...
        with _model_lock:
            if model_mode == "custom":
                return tts_model.generate_custom_voice(
                    ...
                    language=voice_cfg.get("language", "Korean"),
                )
    ...
    if fmt == "pcm":
        return Response(content=_to_pcm16(audio), media_type=content_type)
    return Response(content=_to_wav_bytes(audio, sr), media_type=content_type)
...
def main():
    global tts_model, voices, default_voice, model_mode, SAMPLE_RATE

    args = _parse_args()
    model_mode = "custom" if "customvoice" in args.model.casefold() else "base"

    # Build voice registry
    if model_mode == "custom":
        voices = {
            name: {"speaker": name, "language": "Korean"}
            for name in CUSTOM_VOICE_NAMES
        }
        default_voice = "Sohee"
        logger.info("Enabled %d built-in CustomVoice speakers", len(voices))
    elif args.voices:
    ...
    logger.info("Capturing CUDA graphs for low-latency inference …")
    tts_model.warmup(prefill_len=100)
