...
def commitment(c,external):
    ...
    if external:
        ...
        # v2 reseal: the v1 root's bodies were English, so Korean-first input
        # screening rejected every generated chat and no arm could run.  The
        # receipt therefore has to carry the offline Hangul stream evidence.
        language={'hangul_stream_check':'pass','model_calls':0}
        if set(receipt)!={'schema_version','root_id','sealed_manifest_raw_sha256','validation','language_validation','attestation'} or receipt.get('schema_version')!='airi.e2-c1-blind-validation-receipt.v1' or receipt.get('root_id')!=c['root_id'] or receipt.get('sealed_manifest_raw_sha256')!=sha(read(external/'sealed_manifest.json')) or validation!=required or receipt.get('language_validation')!=language or attestation!=c['pre_result_attestation']:raise FrozenContractError('blind receipt')
