#!/usr/bin/env python3
"""Fail-closed comparator for the frozen D1 four-arm blind broadcast matrix.

Consumes the 48 reports the T3 matrix launcher writes under ``-MatrixProfile
d1`` (``baseline``, ``e2``, ``e2-c1``, ``e2-c2``) and publishes exactly one
verdict file that applies the frozen metric policy.  Nothing here authorizes
adoption: the verdict always carries ``adoption_authorized: false``, and a
winner is only ever named when every gate that applies to that arm passes.

Unlike the single-candidate E2-C1/E2-C2 comparators, D1 evaluates *every* arm:

* zero-violation and perfect-rate gates apply to every arm, using that arm's
  own reports;
* the additive ``minimum`` floors apply to every arm, but the additive
  ``minimum_delta`` requirements and the legacy regression gates apply only to
  the trained candidates in :data:`DELTA_GATED_ARMS`, because those are the only
  arms whose contract is "must improve on ``e2``".  An untrained arm is never
  blocked by a delta it was never asked to produce.
...
explicit launcher attestation instead of inventing a report-shaped proxy.  That
same attestation must additionally carry ``handle_grounding_guard: 'on'`` and
``deterministic_utterance_layer: 'on'``, because the D1 matrix is measured with
both enabled and the report schema has no field for either.
"""
...
SCHEMA_VERSION = 'airi.d1-blind-comparison.v1'
REPORT_SCHEMA_VERSION = 'airi.broadcast-sim-report.v1'
ENVIRONMENT_ATTESTATION_SCHEMA_VERSION = 'airi.d1-environment-attestation.v1'
REFERENCE_ARM = 'e2'
DELTA_GATED_ARMS = ('e2-c1', 'e2-c2')
VERIFIER_PATH = Path(__file__).resolve().parents[2] / 'training' / 'verify_d1_frozen_contract.py'
EPSILON = 1e-9
...
def load_frozen_verifier() -> Any:
    ...
    spec = importlib.util.spec_from_file_location('verify_d1_frozen_contract', VERIFIER_PATH)
...
def assert_arm_rules(policy: dict[str, Any], commitment: dict[str, Any]) -> None:
    """The policy's arm roles must match this comparator's frozen constants."""
    rules = policy.get('arm_rules')
    if not isinstance(rules, dict):
        raise BlindComparisonError('metric policy is missing arm_rules')
    if rules.get('reference_arm') != REFERENCE_ARM:
        raise BlindComparisonError(f'metric policy reference_arm is not {REFERENCE_ARM!r}')
    delta_gated = rules.get('delta_gated_arms')
    if not isinstance(delta_gated, list) or tuple(delta_gated) != DELTA_GATED_ARMS:
        raise BlindComparisonError('metric policy delta_gated_arms is not '
                                   f'{list(DELTA_GATED_ARMS)}')
    arms = commitment.get('arms')
    if not isinstance(arms, list):
        raise BlindComparisonError('blind commitment is missing arms')
    missing = [arm for arm in (REFERENCE_ARM, *DELTA_GATED_ARMS) if arm not in arms]
    if missing:
        raise BlindComparisonError(f'blind commitment does not carry the arms {missing}')


def load_environment_attestation(path: Path, root_id: str, expected_run_count: int) -> dict[str, Any]:
    """The non-report zero_violations and runtime layers must be attested, never assumed."""
    ...
    if document.get('run_count') != expected_run_count:
        raise BlindComparisonError(f'environment attestation run_count is not {expected_run_count}')
    ...
    if document.get('handle_grounding_guard') != 'on':
        raise BlindComparisonError("environment attestation must attest handle_grounding_guard "
                                   "== 'on' for the D1 matrix")
    if document.get('deterministic_utterance_layer') != 'on':
        raise BlindComparisonError("environment attestation must attest "
                                   "deterministic_utterance_layer == 'on' for the D1 matrix")
    return document
...
def legacy_gates_for(arm: str, rates: dict[str, Any], deltas: dict[str, Any],
                     per_role: dict[str, Any], roles: Sequence[str],
                     policy: dict[str, Any]) -> dict[str, Any]:
    """Trained-candidate regression gates: only ever applied to DELTA_GATED_ARMS."""
    legacy_policy = policy['legacy_comparator_gates']
    legacy: dict[str, Any] = {}
    donation = rates[arm]['donation_callout_correct']
    ...
    legacy['memory_probe'] = {
        'candidate_rate': rates[arm]['memory_probe'],
        'reference_rate': rates[REFERENCE_ARM]['memory_probe'],
        'final_blind_minimum': memory_floor,
        'passed': (rates[arm]['memory_probe'] >= rates[REFERENCE_ARM]['memory_probe'] - EPSILON
                   and rates[arm]['memory_probe'] >= memory_floor - EPSILON),
    }
    topic_floor = legacy_policy['topic_anchored']['minimum_delta_vs_e2']
    legacy['topic_anchored'] = {
        'delta_vs_e2': deltas[arm]['topic_anchored'], 'minimum_delta_vs_e2': topic_floor,
        'passed': deltas[arm]['topic_anchored'] >= topic_floor - EPSILON,
    }
    fact_floor = legacy_policy['viewer_fact_usage']['final_blind_minimum_delta_vs_e2']
    per_fixture = {}
    for role in roles:
        candidate = ratio(tuple(per_role[arm][role]['viewer_fact_usage']),
                          f'{arm}/{role}: viewer_fact_usage')
    ...
    legacy['viewer_fact_usage'] = {
        'delta_vs_e2': deltas[arm]['viewer_fact_usage'],
        ...
        'passed': (all(value > EPSILON for value in per_fixture.values())
                   and deltas[arm]['viewer_fact_usage'] >= fact_floor - EPSILON),
    }
    return legacy


def evaluate_arm(arm: str, rates: dict[str, Any], deltas: dict[str, Any], per_role: dict[str, Any],
                 roles: Sequence[str], violations: dict[str, Any], perfect: dict[str, Any],
                 perfect_rates: dict[str, Any], policy: dict[str, Any],
                 attestation: dict[str, Any]) -> tuple[dict[str, Any], int]:
    """Every gate that applies to one arm, plus its improved-additive-axis count."""
    delta_gated = arm in DELTA_GATED_ARMS
    gates: dict[str, Any] = {}
    for name in REPORT_DERIVED_VIOLATIONS:
        gates[f'zero_violations.{name}'] = {
            'source': 'reports', 'candidate_violations': violations[arm][name],
            'passed': violations[arm][name] == 0,
        }
    for name in ATTESTED_VIOLATIONS:
    ...
    for name, expected in sorted(policy['hard_gates']['perfect_rates'].items()):
        gates[f'perfect_rates.{name}'] = {
            'source': 'reports', 'expected': expected,
            'candidate_rate': perfect_rates[arm][name],
            'passed': perfect_rate_ok(tuple(perfect[arm][name]), expected, name),
        }
    improved = 0
    for name, gate in sorted(policy['additive_candidate_gate_vs_e2'].items()):
        axis = gate['metric']
        candidate = rates[arm][axis]
        entry: dict[str, Any] = {
            'metric': axis, 'candidate_rate': candidate, 'minimum': gate['minimum'],
            'passed': candidate >= gate['minimum'] - EPSILON,
        }
        if delta_gated:
            delta = deltas[arm][axis]
            entry['delta_vs_e2'] = delta
            entry['minimum_delta'] = gate['minimum_delta']
            entry['delta_gated'] = True
            entry['improved_vs_e2'] = delta > EPSILON
            entry['passed'] = entry['passed'] and delta >= gate['minimum_delta'] - EPSILON
            if delta > EPSILON:
                improved += 1
        gates[f'additive.{name}'] = entry
    if delta_gated:
        for name, gate in legacy_gates_for(arm, rates, deltas, per_role, roles, policy).items():
            gates[f'legacy.{name}'] = gate
    return gates, improved
...
def evaluate(reports: dict[tuple[str, str, int], dict[str, Any]], policy: dict[str, Any],
    ...
    for arm in arms:
        gates, improved = evaluate_arm(arm, rates, deltas, per_role, roles, violations, perfect,
                                       perfect_rates, policy, attestation)
        per_arm[arm] = gates
        if arm in DELTA_GATED_ARMS:
            improved_axes[arm] = improved

    failed = sorted(f'{arm}:{name}' for arm in arms
                    for name, gate in per_arm[arm].items() if not gate['passed'])

    selection = policy['selection']
    ordered_all = sorted(arms, key=lambda arm: (-scores[arm], arm))
    margin = rounded(scores[ordered_all[0]] - scores[ordered_all[1]])
    eligible = [arm for arm in arms if all(gate['passed'] for gate in per_arm[arm].values())]

    winner: str | None = None
    reason: str | None = None
    if not eligible:
        reason = 'no arm passed every applicable gate; failed gates: ' + ', '.join(failed)
    else:
        provisional = sorted(eligible, key=lambda arm: (-scores[arm], arm))[0]
        tied = sorted(arm for arm in eligible
                      if arm != provisional and scores[arm] == scores[provisional])
        margin = rounded(scores[provisional] - max(scores[arm] for arm in arms if arm != provisional))
        candidate_delta = rounded(scores[provisional] - scores[REFERENCE_ARM])
        if tied:
            reason = (f'exact weighted-score tie at {scores[provisional]} between '
                      + ', '.join([provisional] + tied))
        ...
        elif (provisional in DELTA_GATED_ARMS
                and candidate_delta < selection['delta_gated_minimum_weighted_score_delta_vs_e2'] - EPSILON):
            reason = (f'{provisional} weighted score delta {candidate_delta} is below '
                      f'{selection["delta_gated_minimum_weighted_score_delta_vs_e2"]}')
        elif (provisional in DELTA_GATED_ARMS
                and improved_axes[provisional] < selection['delta_gated_minimum_improved_additive_axes']):
            reason = (f'{provisional} improved {improved_axes[provisional]} additive axes, below '
                      f'{selection["delta_gated_minimum_improved_additive_axes"]}')
        else:
            winner = provisional
    ...
    return {
        ...
        'delta_gated_arms': list(DELTA_GATED_ARMS),
        'axis_rates': rates,
        ...
        'gates': {'per_arm': per_arm, 'failed': failed},
        'scores': scores,
        'score_margin': margin,
        'improved_additive_axes_vs_e2': improved_axes,
        'winner': winner,
...
def publish(output: Path, verdict: dict[str, Any]) -> None:
    ...
    handle_fd, temporary = tempfile.mkstemp(prefix='.d1-', suffix='.json', dir=output.parent)
    try:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description='Frozen D1 four-arm blind matrix comparator')
