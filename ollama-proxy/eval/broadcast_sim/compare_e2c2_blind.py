#!/usr/bin/env python3
"""Fail-closed comparator for the frozen E2-C2 blind broadcast matrix.

Consumes the 36 reports the T3 matrix launcher writes under ``-MatrixProfile
e2c2`` and publishes exactly one verdict file that applies the frozen metric
...
same attestation must additionally carry ``handle_grounding_guard: 'on'``,
because the E2-C2 matrix is measured with the proxy handle-grounding guard
enabled and the report schema has no field for it.
"""
...
SCHEMA_VERSION = 'airi.e2-c2-blind-comparison.v1'
REPORT_SCHEMA_VERSION = 'airi.broadcast-sim-report.v1'
ENVIRONMENT_ATTESTATION_SCHEMA_VERSION = 'airi.e2-c2-environment-attestation.v1'
CANDIDATE_ARM = 'e2-c2'
REFERENCE_ARM = 'e2'
VERIFIER_PATH = Path(__file__).resolve().parents[2] / 'training' / 'verify_e2_c2_frozen_contract.py'
EPSILON = 1e-9
...
def load_frozen_verifier() -> Any:
    ...
    spec = importlib.util.spec_from_file_location('verify_e2_c2_frozen_contract', VERIFIER_PATH)
...
def evaluate(reports: dict[tuple[str, str, int], dict[str, Any]], policy: dict[str, Any],
    ...
    elif top == CANDIDATE_ARM and candidate_delta < selection['e2_c2_minimum_weighted_score_delta_vs_e2'] - EPSILON:
        reason = (f'e2-c2 weighted score delta {candidate_delta} is below '
                  f'{selection["e2_c2_minimum_weighted_score_delta_vs_e2"]}')
    elif top == CANDIDATE_ARM and improved < selection['e2_c2_minimum_improved_additive_axes']:
        reason = (f'e2-c2 improved {improved} additive axes, below '
                  f'{selection["e2_c2_minimum_improved_additive_axes"]}')
    else:
        winner = top
...
def publish(output: Path, verdict: dict[str, Any]) -> None:
    ...
    handle_fd, temporary = tempfile.mkstemp(prefix='.e2c2-', suffix='.json', dir=output.parent)
    try:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description='Frozen E2-C2 blind matrix comparator')
