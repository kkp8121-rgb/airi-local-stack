        ...
        for phase_index, phase in enumerate(("opening", "middle")):
            ...
            for gate in campaign.GATES:
                ...
                report = {
                    ...
                    "surface_signal_counts": {"question_mark": 1},
                    "adjacent_event_count": 1,
                    "adjacent_signal_pair_counts": {},
                    "max_events_in_rolling_5s": 2,
                    ...
                    "flow": {
                        ...
                        "gap_p95_ms": 1_000,
                        "gap_max_ms": 1_000,
                        "interarrival_rate_per_minute": 60.0,
                        ...
                        "noise_rate": 0.0,
                        "eligible_rate": 0.5,
                        "repeat_cluster_count": 0,
                        "max_repeat_cluster_size": 0,
                    },
...
class CampaignAggregationTests(unittest.TestCase):
    ...
    def test_happy_campaign_is_content_free_and_deterministic(self):
        ...
        self.assertEqual(result["source_pattern_aggregates"]["event_count"], 12)
    ...
    def test_source_runtime_gate_and_score_mismatches_fail_closed(self):
        for mode in ("identity", "shared_identity", "capture_reuse", "runtime", "gate", "score", "observation", "source_labels"):
            with self.subTest(mode=mode):
                files = _artifacts()
                if mode == "identity":
