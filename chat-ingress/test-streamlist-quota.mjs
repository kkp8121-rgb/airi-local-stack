...
test('normal close and first response counts are content-free', async () => {
  ...
  assert.deepEqual(result, { schema: 'airi.streamlist-quota-trial.v1', kind: 'message', status: 'completed', stopReason: 'normal_close', durationMs: 0, messages: 2, responses: 1, connections: 1, resumes: 0, firstResponseMs: 0, firstMessageMs: 0 })
...
test('clock and report allowlists reject forged or secret-bearing inputs', async () => {
  ...
  const trial = { schema: 'airi.streamlist-quota-trial.v1', kind: 'idle', status: 'completed', stopReason: 'bad reason', durationMs: 1, messages: 0, responses: 0, connections: 1, resumes: 0, firstResponseMs: null, firstMessageMs: null }
...
test('report state and caller signal contracts are exact', async () => {
  ...
  const base = { schema: 'airi.streamlist-quota-trial.v1', kind: 'idle', status: 'completed', durationMs: 1, messages: 0, responses: 0, connections: 0, resumes: 0, firstResponseMs: null, firstMessageMs: null }
...
test('manual quota validation and report aggregation reject forbidden data', () => {
  const trial = { schema: 'airi.streamlist-quota-trial.v1', kind: 'idle', status: 'completed', stopReason: 'normal_close', durationMs: 1, messages: 0, responses: 0, connections: 1, resumes: 0, firstResponseMs: null, firstMessageMs: null }
