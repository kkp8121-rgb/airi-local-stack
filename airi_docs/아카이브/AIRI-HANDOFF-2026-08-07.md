...
## 2026-08-07 종료 시점 추가 인계

- 실제 마이크 검색 선반응은 적용 중간에 `1.460s`, `1.620s`, `1.850s`,
  `1.552s`, `1.500s`를 기록해 모두 2초 이하였다. 다만 마지막 두 번 중 첫 턴은
  문장 첫 `음유잉여`가 누락됐으므로 이 수치만으로 최종 완료 판정하지 않는다.
- 설치 AIRI의 VAD speech pre-roll을 `360 → 600ms`로 늘렸다. 이는 문장 시작
  오디오를 240ms 더 보존하며, 발화 종료를 기다리는 450ms silence 값은 바꾸지 않는다.
- 검색 명령에서 AIRI의 `[날짜 시간]` 접두사를 제거한다. `웹에서 검색해줘`만
  인식된 경우 최근의 명시적 검색어를 복구하며, 이전 검색어가 없으면 임의 검색하지 않는다.
- 일반 대화 `오늘 날씨 어때` 2회 중 첫 턴은 Whisper가 실제 발화보다 긴 문장을
  생성해 `implausible_text_rate`로 제거됐다. 두 번째 턴은 실제 첫 재생이 발화 종료
  추정 후 **1.514초**였다. 길이 제한을 완화하지 않고, 이런 실패 턴에만 beam 3,
  내부 VAD 없는 보수적 2차 디코딩을 한 번 수행하도록 추가했다. 실제 마이크 재검증은 남았다.
- 최신 자동 테스트는 STT 30 + proxy 29 + TTS 21 + monitor 5 = **85개**다.

### 제품 방향 — 다음 세션에서 반드시 유지

`음유잉여` 검색은 지연·고유명사·도구 호출을 동시에 검증하는 테스트 케이스다.
최종 제품은 검색 봇이 아니라 뉴로사마처럼 방송 가능한 사람형 캐릭터다. 검색 전용
고정 ACK를 범용 대화 구조로 확대하지 않는다.

다음 구조를 공통 대화 계층으로 설계한다.

1. 2초 이내 감정·맞장구·되묻기를 담당하는 빠른 반응층
2. 최근 맥락을 판단하는 대화 두뇌와 도구 라우터
3. `최근 검색어 + 반복 횟수 + 마지막 질문 시각`을 포함한 단기 상태
4. 장기 기억과 캐릭터 관계·관심사
5. 화면·게임·채팅 관찰, 자발적 발화와 침묵 관리를 담당하는 방송 행동층

같은 검색어가 반복되면 고정적으로 재검색하지 말고 “계속 찾고 있네. 무슨 일이
있어?”처럼 되묻는 것이 목표다. 이 반응은 검색 예외처리가 아니라 모든 반복 발화에
적용 가능한 공통 단기 대화 상태에서 만든다.
