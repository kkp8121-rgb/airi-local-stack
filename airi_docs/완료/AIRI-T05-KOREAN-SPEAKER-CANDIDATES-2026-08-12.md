...
청취 비교용이며 126번만 후속 비교 후보로 선택됐고 운영 화자로 승인되지는
않았다. attribution 의무와
...
이 dev PC의 local-only 샘플 생성과 사용자 1차 청취는 완료했다. 사용자는
`reference-126-126_003_2800`을 **한국어 화자 예비 후보**로 선택했다. 126번은
기계음처럼 들리지는 않지만 말투가 딱딱하고 감정이 적어 낭독처럼 들린다는
평가다. 현행 일본어 참조 교차클로닝은 한국어가 다소 어눌하고 일본어 화자의
한국어처럼 들리지만, 비교상 방송인다운 개성이 더 낫다는 평가를 받았다.

따라서 이번 판정은 **126번 보존, 운영 승격 보류, 현행 음성 유지**다. 126번은
나중에 감정이 드러나는 대화체 참조·문장으로 다시 합성해 현행 음성과 A/B할
때 우선 사용한다. fresh checkout에는 WAV가 없으며, 검증 종료 후 서비스는
기존 일본어 참조와 TTS cache 7/7 상태로 복구돼 있다.
