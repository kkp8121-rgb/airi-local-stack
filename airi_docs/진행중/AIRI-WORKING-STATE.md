---
schema_version: 1
updated_at_kst: "2026-08-26 17:32:00 +09:00"
checkpoint_id: "20260826-174000-m7-s1-correction"
matrix_note: "M7 S1 corrected: cutting all three feedback channels removes the template lock (0/3) but a phrase lock remains in 2/3 runs (top response 28-33/99, e.g. the opening beat cue paraphrased); Claude reference rating of 06b-r3 = 3-axis 1.59 vs human baseline 1.76 (run 05 ref 1.72) — no improvement, failure mode swapped. Root cause reframed: context-poor replay input (chat reacting to the original stream) + model default; feedback only amplifies. Production flag stays off, no adoption request. Next candidate S6 input alignment (user decision). Waiting on human rating + push approval. No services running"
...
active_phase: "m7-await-human-rating-run05-06b-r3"
git_head: "29181bac97f97cfce380f72f0954f88fbef31d3f"
...
| 현재 행동 | S0·S1 실행·기록 완료. 사용자 채점(06b-r3·run 05)과 추가 commit push 승인을 기다린다. 실행 중 프로세스 0. |
| 다음 성공 조건 | M7-3 사람 채점으로 참고 채점(06b-r3 1.59 < 1.76) 확인. 개선 없음이 확정되면 S6 입력 정합(자기완결 구간 선별 vs 스트리머 STT 발화 정렬)을 사용자가 고르고, 그 위에서 복제 3회. |
