---
schema_version: 1
updated_at_kst: "2026-08-26 10:10:11 +09:00"
checkpoint_id: "20260826-101011-m4-claude-handoff-final-publication-intent"
matrix_note: "M4 complete after final publication transaction. D1v6 terminal/no_winner, campaign N/A, dashboard skill/audit/final validation complete. User approved one final push and requested all milestone docs plus a Claude POST-M4 handoff. Parent HEAD 09de314 is ahead of origin/main 3 and was clean before this docs batch; adoption remains forbidden and M5 starts only if the user submits the handoff goal to Claude"
...
active_phase: "m4-complete-claude-handoff-publication"
git_head: "09de314c84c2a075ddaab99e0bab4968a04b78f0"
worktree_state: "publication basis: parent HEAD/local main 09de314 was ahead of origin/remote 30fe352 by 3 and clean before the exact final documentation batch. This checkpoint is published with the current milestone docs and new Claude handoff in the user-approved single push; final readers must verify actual refs and clean state because the final commit cannot self-reference. d1v6 terminal no_winner; wrapper/owned process/listener 0; campaign N/A; adoption forbidden"
...
- 2026-08-25 21:40 KST **메인 세션 Codex 이관 receipt**: 사용자 선언("이제 메인 세션은 코덱스")에
  ...
  `C:\Projects\airi`와 `D:\AIRI-Models`를 더 이상 변경하지 않으며, 실행 중 프로세스·Monitor·
  subagent를 남기지 않는다(matrix 0, 서비스 0, 병렬 세션 airi-66 idle·branch 병합 완료). Codex의
  단일 진입점은 `AIRI-CODEX-HANDOFF-2026-08-25-M4.md`이고 M4 착수는 사용자 `/goal`(Codex CLI
  `codex -C C:\Projects\airi -a on-request -s workspace-write "…"`) 이후다. adoption=false 유지.
...
- 2026-08-25 17:35 KST **d1v5 48-report matrix intent (exact-once)**: 봉인·배선 receipt commit
  ...
  392 MiB 유휴, Ollama에 4 arm tag/digest 전부 일치(model manifest sha `050ae10f…e330`, D1과
  동일 파일을 새 root로 복사). 새 root `D:\AIRI-Models\airi-d1v5-blind-matrix-20260825\`에
  wrapper `launch-d1v5-matrix.ps1`(sha `64c96fa3…`, D1 wrapper와 동일 구조: 종료 시
  `launcher.exit-code.txt` no-overwrite 기록)를 두었고 `run\`은 미생성이다.
  **exact command(정확히 한 번)**: `"C:\WINDOWS\System32\WindowsPowerShell\v1.0\powershell.exe"
  -NoProfile -File D:\AIRI-Models\airi-d1v5-blind-matrix-20260825\launch-d1v5-matrix.ps1` →
  내부적으로 `run-airi-broadcast-t3-matrix.ps1 -MatrixProfile d1v5 -OutputDir …\run
  -ModelManifest …\d1-model-manifest.json -BlindRoot D:\AIRI-Models\airi-d1-blind-freeze-20260825-v5`
  (num_ctx 4096 기본값, guard+layer ON, 브리핑 마커 포함 프롬프트). 시작 후 PID·시작 시각을
  `launcher-start-receipt.json`으로 남기고 14분 상한 heartbeat로 read-only 감시한다.
  예상 3.5~4시간. 실패 시 root 보존·같은 명령 반복 금지. blind v5는 이 실행으로 소비된다.
  GPU 학습 0, 운영 태그 변경 0, adoption 0.
...
- 2026-08-25 17:40 KST **M3 항목 1·2 receipt (commit 직전)**:
  - **native_baseline 핀 수리**: `run_airi_native_baseline.py` `CASES_SHA256`을 커밋 blob SHA
    `95309e101e30009ec12a9edb4d047e69a46aec42c2f46df0a8b70962225dc3be`로 교체(픽스처 바이트
    변경 0, 주석에 경위 기록). `test_airi_native_baseline.py` **4 passed**(이전 1 failed).
  - **일반 능력 게이트 강제**: 신규 `ollama-proxy/training/verify_general_capability_gate.py`
    (JSON만 읽음, 모델 로드 0, threshold CLI 완화 불가, group/subtask 누락·harness 설정 불일치
    fail-closed), 기준선 commitment `fixtures/commitments/airi_general_capability_baseline.json`
    (schema `airi.general-capability-baseline.v1`, stock kobest 0.6838/haerae 0.7195 + 10
    subtask, stock results sha `f58250c9…9f9d`, 예산 2.0%p), 테스트 10건, CI shard
    `ollama-proxy-training` 등록. 실제 results 3건 실행: stock pass(−0.00/−0.00), v3 pass
    (0.52/1.19), e2c2 pass(0.89/1.92) — 모두 exit 0. 첫 실행은 lm_eval이 `batch_size`를 문자열
    `'16'`로 기록해 타입 불일치로 거부됐고 문자열 정규화 비교로 고쳤다(테스트 추가).
  - **blind v5 저작 위임 중**: seal 도구에 `d1v5` generation(v3·v4 superseded 확장, 기존 d1
    commitment 테스트 18 passed 유지) 추가 후, opus 구현자 subagent가 외부 staging root
    `D:\AIRI-Modelsiri-d1-blind-staging-v5\`에 새 한국어 fixture 3종을 저작하고 `--check`로
    검증 중(봉인·git·모델 호출 금지). 완료 보고를 받으면 pins를 대조하고 직접 봉인한다.

- 2026-08-25 17:25 KST **M3 goal intent (사용자 `/goal`: "모두 추천 방향으로 승인 작업 진행")**:
  승인된 3건을 아래 순서로 수행한다. 착수 관측: HEAD/local/origin `bae4e4a` exact, worktree
  clean, AIRI 서비스 0, GPU 유휴.
  1. **native_baseline 픽스처 핀 수리**: `git log`로 `airi_baseline_cases.json`은 `a7412af`
     (08-10) 이후 단 한 번도 바뀌지 않았고(blob `95309e10…3be`), 핀 `f3fd3d21…`은 `565cc65`
     (08-14)에서 추가된 뒤 **어떤 커밋 blob과도 일치한 적이 없다**. 픽스처가 원본이고 핀이
     틀린 것이므로 `CASES_SHA256`을 커밋 blob SHA로 고친다. 픽스처 바이트 변경 0.
  2. **일반 능력 게이트 강제**: 오프라인 검증기 `ollama-proxy/training/verify_general_capability_gate.py`
     + 기준선 commitment(`fixtures/commitments/airi_general_capability_baseline.json`: stock
     kobest 0.6838 / haerae 0.7195, results SHA) + 테스트 + CI shard 등록. 규칙: 후보 lm-eval
     results JSON의 두 group acc가 stock 대비 **2.0%p 초과 하락이면 fail-closed**. 실행은
     merge/package 직후 수동 1회(패키저 코드 변경 없음).
  3. **blind v5 봉인 → 4-arm 재측정**: seal 도구에 `d1v5` generation(v3·v4 superseded 확장),
     launcher `d1v5` 프로파일(d1과 동일 게이트·attestation·comparator, root_id/commitment만
     v5), commitment/policy(threshold 값 D1과 동일, 완화 0), 계약 테스트. 새 한국어 fixture
     3종은 v1~v4 handle/template/hash 재사용 0으로 **외부 staging root**에서 저작하고
     `--check`로 오프라인 검증 후 봉인한다. 봉인 후 별도 intent를 쓰고 detached wrapper로
     matrix를 **정확히 한 번** 실행, 14분 heartbeat로 감시, verdict 분기(winner → F7 게이트
     통과한 campaign / no_winner → 진단·대기).
  경계: GPU 학습 0, blind v1~v4 재사용 0, threshold·seed·fixture 정의 변경 0, 운영 모델·
  태그 변경 0, 외부 provider 0, adoption 0(campaign 결과와 무관).
...
- 2026-08-25 17:05 KST **M2 배치 B receipt — goal 완료**: greybox 평가 툴 2종 도입·측정
  ...
  완료 절 등록.
  - **llama-perplexity**(핀 b10375, CPU, ctx 2048, 24 chunk): v3 `airi-bf16.gguf` PPL
    **4.2542 ± 0.068**, `airi-q4_k_m.gguf` **4.3310 ± 0.070**, **비율 1.0181**(제안 기준
    ≤1.05 안). 텍스트는 v3 assistant 발화(학습 데이터, 비율만 의미), blind 미사용.
  - **lm-eval 0.4.12**(kobest+haerae 전량, 0-shot, bf16, batch 16, seed 42, 모델당 ~7분,
    exit 0×3): kobest stock 0.6838 / v3 0.6786(−0.53%p) / e2c2 0.6749(−0.90%p); haerae
    0.7195 / 0.7076(−1.19%p) / 0.7003(−1.92%p). 누적 병합마다 **단조 하락**, 크기는 제안
    게이트 ≤2%p 안이지만 e2c2 haerae는 경계. `kobest_wic`는 세 모델 동일(변별력 0).
    results JSON sha `f58250c9…`/`4a5f0fc9…`/`a93df885…`, 요약 `lmeval-summary.json`
    `70b9d126…`. 게이트 값은 제안만 하고 강제하지 않았다.
  - 도입 안 함: Unsloth(핀 비호환)·TRL DPO(쌍 데이터 없음).
  - **goal 결산**: 제안 3건(문서 편입 `6450a96`, num_ctx 4096+예산 관측 `dd327cb`, F7 게이트
    `19e558f`) + 이로운 툴 2종 도입·기준선 박제 완료. 경계 준수: GPU 학습 0, blind 재사용 0,
    matrix 재실행 0, 태그 재패키징 0, 외부 provider 0, adoption 0. GPU 370 MiB(유휴), AIRI
    서비스 0, owned 포트 free. **남은 결정(사용자)**: ① 재측정용 새 blind(v5) 봉인·4-arm
    matrix 재실행 여부(프롬프트 마커·num_ctx 변경으로 과거 blind와 비교 불가), ② 일반 능력
    게이트(≤2%p) 강제 여부, ③ `test_airi_native_baseline` 픽스처 핀 불일치(기존 CI 실패) 처리.

- 2026-08-25 16:27 KST **M2 배치 B intent — greybox 평가 툴 2종 도입**: 배치 A는
  `6450a96`·`dd327cb`·`19e558f`·`50041f1` 네 commit으로 push됐고 HEAD/local/origin exact,
  worktree clean이다. (정정: 위 17:20 receipt의 native_baseline 실패 원인 추정을 "줄바꿈"에서
  "커밋된 blob SHA 자체가 핀과 불일치"로 고쳤다.)
  - 산출물 root: `D:\AIRI-Modelsiri-m2-greybox-eval-20260825\` (신규, Git 미포함).
  - **lm-evaluation-harness**: `D:\AIRI-Modelsenv-lmeval`(uv, cpython 3.12, `lm_eval` +
    torch CUDA) 별도 venv. 대상은 stock `midm-2.0-mini-instruct-hf`와 v3 `merged-hf`, 그리고
    최신 학습본 `airi-e2-c2-merged-20260824\merged-hf`. 태스크 `kobest`·`haerae`·`kmmlu`를
    동일 seed 42·`--limit`(런타임 실측 후 고정)·bf16으로 측정. 게이트 값은 확정하지 않고
    기준선만 박제한다. 학습 0, 가중치 수정 0.
  - **llama.cpp `llama-perplexity`**(핀 b10375, `D:\AIRI-Models\llama-b10375-bin-win-cpu-x64`,
    CPU): v3 `package-q4iri-bf16.gguf` vs `airi-q4_k_m.gguf`를 같은 텍스트로 측정해 Q4/bf16
    비율을 기록. 텍스트는 blind가 아닌 v3 chat jsonl의 assistant 발화(학습 데이터라 절대값이
    아니라 **비율**만 의미) — blind v1~v4 미사용.
  - 긴 명령은 background로 돌리고 로그를 root에 남긴다. GPU 사용은 lm-eval 추론뿐이다.

- 2026-08-25 16:24 KST **M2 배치 A receipt (검증 완료, commit/push 직전)**: intent 항목
  ...
  - **기존 실패 기록**: `eval/test_airi_native_baseline.py::test_pinned_fixture_has_sixteen_cases`
    는 변경 전 HEAD(stash 대조)에서도 실패 — 커밋된 픽스처 blob SHA `95309e10…` ≠ 핀 `f3fd3d21…`(HEAD blob과
    작업 트리 동일, 줄바꿈 문제 아님). CI matrix 등록 파일이라 별도 배치 과제로 남긴다. 이 배치에서 고치지 않았다.
...
- 2026-08-25 16:05 KST **M2 goal intent (사용자 `/goal`: "제안 방향대로 진행하고 도입해서
...
- 2026-08-25 14:48 KST **P3 브리핑 마커 receipt (사용자 승인안 (a) 구현 완료)**:
...
- 2026-08-25 14:43 KST **프록시 오류 근본 원인 확정 receipt — Ollama 400
...
- 2026-08-25 14:30 KST **프록시 오류 재현 실행 intent (사용자 승인)**: 사용자가
...
- 2026-08-25 13:10 KST **M1 계측 복구 receipt (부분 완료 + 근본 원인 확정)**:
...
- 2026-08-25 13:00 KST **M1 계측 복구 intent (사용자 지시)**: D1 종결 뒤 사용자가
...
- 2026-08-25 10:27 KST **Claude 이관 다섯 문서 갱신 intent**: monitor 종료 receipt
  ...
  75,211 B, stderr 0 B, exit receipt absent다. HEAD/local/origin-main은 `d5bab18`
  exact다. WORKING, `AIRI-D1-CODEX-HANDOFF-2026-08-25.md`, ROADMAP-STATUS,
  NEXT-SESSION, ROADMAP-LOG 다섯 이관 문서만 Claude 단일 진입점·현 진행률·후속 분기
  책임으로 갱신한다. work-continuity와 diff-check가 PASS한 경우에만 이 다섯 경로를
  exact stage해 `docs: hand off D1 matrix supervision`으로 commit/push한다. 실패 시
  reset/checkout/revert/force/retry 없이 문서 diff와 live matrix를 보존한다. Codex는
  이관 후 matrix 감시·verdict·campaign을 수행하지 않는다.

- 2026-08-25 10:26 KST **Codex monitor 종료 receipt / detached wrapper 불변**:
  `/root/monitor_d1_matrix`만 interrupt했고 agent status가 `interrupted`임을 확인했다.
  직후 10:25:32 KST read-only 재관측은 wrapper PID 7832와 exact command가 intent와
  동일하게 live, reports/health 14/48, stdout 70,511 B, stderr 0 B, exit receipt
  absent다. matrix launcher·wrapper·서비스에는 stop/restart/retry/control 0이며 blind
  v4 재사용 0이다. 다음 행동은 현행 handoff/ROADMAP-STATUS/NEXT-SESSION/ROADMAP-LOG를
  읽고 Claude가 즉시 이어받을 수 있는 이관 상태로 문서만 갱신하는 것이다.

- 2026-08-25 10:25 KST **D1 실행 감독 Claude 이관 intent**: 최신 사용자 명령에
  따라 detached matrix 자체는 중단·재실행·재시도하지 않고 감독권과 후속 verdict
  분기만 Claude Code 세션으로 이관한다. 10:24:32 KST read-only 관측은 wrapper PID
  7832 live, exact command `"C:\WINDOWS\System32\WindowsPowerShell\v1.0\powershell.exe"
  -NoProfile -File D:\AIRI-Models\airi-d1-blind-matrix-20260825\launch-d1-matrix.ps1`,
  reports/health 14/48, stdout 70,511 B, stderr 0 B, exit receipt absent다. HEAD/local
  main/origin-main은 `d5bab18f00450c63bc2131ee246f0c8b1f73e76e` exact이고 기존
  WORKING/ROADMAP-LOG heartbeat·push receipt diff만 dirty다. 다음 상태 변경은
  `/root/monitor_d1_matrix` Codex subagent만 interrupt하는 것이며 PID 7832, launcher,
  wrapper, 서비스에는 어떤 제어 신호도 보내지 않는다. blind v4는 소비됐고 재사용·
  재실행 금지다. interrupt receipt 후 다섯 이관 문서를 Claude 단일 진입점 기준으로
  갱신하고 work-continuity/diff-check, exact docs stage, commit/push로 종료한다.

- 2026-08-25 10:18 KST **D1 baseline receipt docs push receipt**: work-continuity
  PASS, diff-check 0 후 두 문서만 commit `d5bab18f00450c63bc2131ee246f0c8b1f73e76e`
  (`docs: record D1 baseline arm receipt`)로 고정하고 `git push origin main` exit 0,
  `a6d275e..d5bab18 main -> main`을 확인했다. HEAD/local/origin-main exact, worktree
  clean이며 matrix는 reports/health 12/48, wrapper live, stderr 0 B다.

- 2026-08-25 10:18 KST **D1 baseline receipt 문서 publish intent**: 첫 3 report
  push receipt, 첫 continuity receipt, heartbeat, baseline arm receipt를 담은 WORKING/
  ROADMAP-LOG 두 경로만 dirty다. work-continuity와 diff-check 후 두 경로만 stage해
  `docs: record D1 baseline arm receipt`로 commit/push한다. 실패하면 force/retry 없이
  로컬 상태를 보존하고 matrix launcher는 재호출하지 않는다.

- 2026-08-25 10:17 KST **D1 baseline arm batch receipt**: baseline reports 12/12,
  health 12/12, run-contract evidence 12/12이며 total reports/health는 12/48이다. root가
  12개 health 전부를 읽어 guard/layer before/after가 모두 true임을 확인했다. wrapper
  PID 7832 live, stderr 0 B, exit receipt absent이며 다음 e2 arm으로 넘어간다. report
  내용의 최종 gate 평가는 48/48 comparator까지 유보하고 launcher 재호출·계약 변경은
  하지 않는다.

- 2026-08-25 10:12 KST **D1 matrix heartbeat**: exact wrapper PID 7832 live,
  reports/health 11/48, stdout 53,525 B, stderr 0 B, exit receipt absent다. report/health
  수가 일치하고 새 오류·재시작·launcher 재호출 0이며 동결 계약을 유지한다.

- 2026-08-25 09:58 KST **D1 matrix heartbeat**: exact wrapper PID 7832 live,
  reports/health 7/48, stdout 35,513 B, stderr 0 B, exit receipt absent다. 새 오류·재시작·
  launcher 재호출 0이며 blind v4 소비 상태와 동결 계약을 그대로 유지한다.

- 2026-08-25 09:47 KST **D1 첫 continuity report attestation receipt**: baseline/
  long_continuity_and_stale_transition/73 report와 health가 생성돼 reports/health 5/48이다.
  root가 저장된 report를 동결 comparator derivation으로 직접 재계산한
  `stale_transition_clean`은 3/10으로 분모 10 > 0이고, 대응 health의 guard/layer는
  before/after 모두 true다. 이로써 첫 identity fixture에서 확인한 identity 9,
  donation 18, decoy 18과 함께 네 perfect-rate 분모의 조기 attestation을 끝냈다.
  이는 조기 분모·환경 확인이지 최종 gate 판정이 아니며 threshold·metric·seed·fixture
  변경은 0이다. wrapper PID 7832 live, stderr 0 B, exit receipt absent; exact-once run을
  14분 이내 heartbeat로 계속 감시한다.

- 2026-08-25 09:46 KST **D1 첫 3 report docs push receipt**: `git push origin
  main` exit 0, `23d0417..a6d275e main -> main`; HEAD/local main/origin-main은 모두
  `a6d275ed1b2b4ef8842a8e52de7d3a9b0e65c0f2`로 exact이고 worktree clean이다.
  wrapper PID 7832 live, reports/health 4/48, stderr 0 B, exit receipt absent다. 첫
  continuity report를 기다려 stale 분모와 두 flag를 검증하며 launcher 재호출은 없다.

- 2026-08-25 09:45 KST **D1 첫 3 report docs commit receipt + push intent**:
  exact 두 문서 배치를 commit `516acf20371258f5819d651b60d6e5ff7a4028f1`
  (`docs: record D1 early matrix attestations`)로 고정했다. commit exit 0, 32 insertions/
  7 deletions, worktree clean, local main은 origin/main `23d0417`보다 1 ahead다. wrapper
  PID 7832 live, reports/health 4/48, stderr 0 B, exit receipt absent다. 이 receipt를 같은
  두 문서로 commit한 뒤 두 commit을 `git push origin main` 한 번으로 push한다. push
  실패 시 force/retry/launcher 재호출 없이 로컬 commit과 실행 root를 보존한다.

- 2026-08-25 09:44 KST **D1 첫 3 report receipt 문서 publish intent**: wrapper PID
  7832 live, reports/health 4/48, stderr 0 B, exit receipt absent다. 첫 3 조기 검증을
  기록한 WORKING/ROADMAP-LOG 두 경로만 dirty이고 work-continuity PASS, repo diff-check
  exit 0이다. 두 경로만 stage해 cached diff-check 후 `docs: record D1 early matrix
  attestations`로 commit하고 origin/main에 push한다. 실패하면 force/retry 없이 로컬
  상태를 보존하며 matrix launcher는 재호출하지 않는다.

- 2026-08-25 09:42 KST **D1 첫 3 report 조기 attestation receipt**: wrapper PID
  7832 live, reports 4/48, 첫 3개에 대응하는 health 증거 3개, stdout 15,231 B,
  stderr 0 B, exit receipt absent다. 저장된 첫 3 report를 comparator의 동결된
  `PERFECT_RATE_DERIVATIONS`로 직접 재계산한 분모는 identity 9, donation 18,
  decoy 18이며 모두 nonzero다. 세 health 증거 모두
  `handle_grounding_guard=true`와 `deterministic_utterance_layer=true`를 before/after에서
  attest했다. `stale_transition_clean` 분모는 0인데, launcher의 동결된 순서상 첫 4개가
  같은 identity fixture의 seed이고 stale metric은 continuity fixture report에서만
  수집되기 때문이다. threshold·metric·seed·fixture·run 순서 변경 없이 첫 continuity
  report에서 stale 분모 nonzero와 두 flag를 확인한 뒤 14분 이내 heartbeat 감시를
  계속한다. 같은 launcher/wrapper 재호출은 0이다.

- 2026-08-25 09:29 KST **D1 matrix detached launch receipt**: intent commit `310e81a`
  push 후 2026-08-25T09:28:26.8479699+09:00에 hidden wrapper를 정확히 한 번 시작했다.
  PID 7832 command는 고정 wrapper exact, start receipt SHA
  `2be69835ac4754c1ee892f613e4e1c11c6480c4081181e2a879f9a157681fdc6`이다.
  09:28:48 관측에서 wrapper live, run dir 생성, stdout 677 B/stderr 0 B, report 0/48,
  first-run owned 11435/8892 listener가 startup 중이다. exit receipt는 아직 없다. **blind
  v4는 이 실행으로 소비되며 재실행 금지다.** 다음 동작은 PID/로그를 건드리지 않고 첫
  report 3개까지 감시해 분모와 두 flag before/after를 조기 attest하는 것이다. heartbeat
  상한 14분, 같은 launcher/wrapper 재호출 0을 유지한다.
...
- 2026-08-25 09:26 KST **D1 PreflightOnly receipt + detached wrapper intent**: external
  ...
  `050ae10f86cef5801b625a54bdfaa136ad927873e62b0e44226fa241b38ae330`을 작성했다.
  d1 preflight exit 0: runs 48, unique keys 48, arms baseline/e2/e2-c1/e2-c2,
  seed_sets 3개×4, comparisons d1-blind, OutputDir/run absent. blind v4 pin과 네 Ollama
  digest는 exact이며 응답 생성 0이므로 아직 미소비다. 이 receipt를 장기 문서 5종에
  commit/push한 뒤 external root에 launcher exact args와 no-overwrite exit-code receipt를
  쓰는 `launch-d1-matrix.ps1` wrapper를 생성·AST/SHA 검증한다. wrapper 자체는 matrix를
  실행하지 않는다. wrapper receipt 뒤 별도 matrix intent를 commit/push하기 전에는
  Start-Process를 호출하지 않는다.

- 2026-08-25 09:25 KST **D1 preflight manifest intent**: launcher 장기 문서 finalization
  `e57d571`도 origin/main에 push돼 HEAD/local/origin-main exact·clean이다. external root
  `D:\AIRI-Models\airi-d1-blind-matrix-20260825`는 absent다. 이 root만 새로 만들고 인계의
  4 arm tag/digest exact JSON을 `d1-model-manifest.json`으로 no-overwrite 생성한다.
  생성 뒤 file size/SHA와 네 Ollama tag/digest를 재대조하고, OutputDir `...\run`이 없는
  상태에서 launcher를 `-MatrixProfile d1 -PreflightOnly`로 한 번 실행한다. 성공 조건은
  runs=48, 48 unique keys, arm 순서 4개, seed sets 3×4, comparison d1-blind, OutputDir
  미생성, blind sealed pin 불변이다. 실패하면 root/manifest를 보존하고 matrix를 시작하지
  않는다. 이 preflight는 응답 생성과 blind 소비가 아니다.

- 2026-08-25 09:23 KST **D1 launcher push receipt + preflight 문서 finalization intent**:
  launcher `9e6b1f4`와 receipt `a510004`를 `7314167..a510004 main -> main`으로
  `git push origin main` 한 번에 push했고 HEAD/local/origin-main exact·worktree clean이다.
  external D1 matrix root/manifest/run은 모두 absent, blind v4 두 pin exact, 11435/
  12393/12394 listener 0이며 Ollama 네 평가 tag/digest가 인계 manifest 값과 전부 exact다.
  launcher 완료 상태를 handoff/ROADMAP-STATUS/NEXT-SESSION/LOG/WORKING 다섯 문서에
  반영해 `docs: publish D1 launcher receipt`로 commit/push한다. 그 뒤에만 external
  root와 model manifest를 생성하고 `-PreflightOnly`를 실행한다. preflight 성공 조건은
  48 unique run keys, arm 순서 baseline/e2/e2-c1/e2-c2, comparison d1-blind, blind v4
  commitment/root/sealed binding exact이며 OutputDir/run과 blind 응답은 생성하지 않는다.

- 2026-08-25 09:22 KST **D1 launcher commit receipt + push intent**: exact 4-path commit
  `9e6b1f4b76eb9d0a8e97a5dec32c82b7792f0424`(`fix: add D1 matrix launcher profile`)
  exit 0, 212 insertions/43 deletions. commit 직후 worktree clean, local main은
  origin/main `7314167`보다 1 ahead, 관련 PID 0이다. 이 receipt를 WORKING/ROADMAP-LOG
  두 경로로 commit한 뒤 두 commit을 `git push origin main` 한 번으로 push한다. push
  실패 시 force/retry/matrix 없이 로컬 commit을 보존한다.
