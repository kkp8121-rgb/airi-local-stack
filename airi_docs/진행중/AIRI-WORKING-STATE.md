---
schema_version: 1
updated_at_kst: "2026-08-26 17:25:00 +09:00"
checkpoint_id: "20260826-172500-m7-s0-s1-receipt"
matrix_note: "M7 S0/S1 done: 13 replay runs (off x4, on x3, journal x3, on+no-briefing-self-echo x3), all exit 0, pickup deterministic, generation not reproducible under the harness seed. Collapse (um>=40 or ?>=60): off 3/4, on 1/3, journal 2/3, on+noEcho 0/3. Proxy flag AIRI_FEEDBACK_HYGIENE merged (default off). Human rating sheets ready: 06b-r3 (pre-registered median), 06a-r1, run 05. Waiting on the user: ratings, push approval for the new commits, later production flag approval. No services running"
...
git_head: "f748caecf4638cb5d9d33253a94eaa48c350ebd4"
...
| 다음 성공 조건 | M7-3: 06b-r3 사람 채점이 run 04 기준선 대비 3축 합성 +0.2 이상(`summarize --baseline`)이면 운영 flag on·하니스 플래그 승인 요청; 미만이면 S2를 06b 구성 위에서 복제 3회. |
