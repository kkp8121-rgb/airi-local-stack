...
3. **S1은 사람 채점 축을 개선하지 못했다(참고 채점 기준).** 06b-r3 3축 합성 1.59는 run 04 기준선(1.76)·run 05(1.72)보다 낮고
   filler 77%다. 실패 모드가 "되묻기"에서 "질문 무시 + 비트 큐 되풀이"로 바뀌었을 뿐이다. 판정은 사람 채점(M7-3)이 하지만, 참고 채점
   방향이 이렇게 뚜렷하면 운영 flag on 승인 요청을 낼 근거가 없다. 채점 대상은 사전 등록대로 **`06b-r3`**(`rating-sheet.html`, 99턴)이며
   run 05 시트도 남아 있다. 보조 `06a-r1/rating-sheet.html`.
