...
- [~] **G5. 자발 행동·방송 디렉터**
  ...
  - [~] B4c 방송 발화 계약 — 수신자 지향 구조(인용→반응 2박자·문체
    ...
    생산한다. 게이트 경로 로컬 실측(2026-08-18)도 같은 결론이다 — 태그의문
    개선은 게이트 위에서도 유지(5→15%/6→18%)되고, 잔존 addressee 4케이스
    (dn04·gr01·sp01·b18)는 게이트+계약 조합에서도 불변이라 결정론 렌더러
    이관의 최종 근거가 됐다 (`완료/AIRI-B4C-GATE-PATH-AB-2026-08-18.md`).
  - [~] 스타일 게이트 방송 경로 배선 확인 — 코드 실측 완료 2026-08-14: 게이트는
    ...
    직접 호출한 설계 차이로 설명됨(방송 경로 우회 아님). 게이트 경로 로컬
    실측 완료 2026-08-18 — 존댓말 축 종결(리허설 위반 0·반말 100%), 계약과의
    이중 배선 실증 (`완료/AIRI-B4C-GATE-PATH-AB-2026-08-18.md`). 라이브(B1b)
    실증 잔여.
