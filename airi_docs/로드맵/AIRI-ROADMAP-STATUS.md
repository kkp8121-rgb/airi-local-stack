...
- M8 처리 종료율: **6/11 (54.5%)** — `([x] + [F]) / 활성 항목 전체`
- M8 진행 지수: **6/11 (54.5%)** — `([x] + [F] + 0.5×[P] + 0.5×[~]) / 활성 항목 전체`
- 전체 로드맵 정상 완료율: **74/112 (66.1%)**
- 전체 로드맵 처리 종료율: **85/112 (75.9%)**
- 전체 로드맵 진행 지수: **88.5/112 (79.0%)** — 127개 행 중 `[S]` 12개와 `[N/A]` 3개를 분모에서 제외; 2026-09-01 15:40 KST 기준. 직전 M7 요약(정상 11/15·처리 13/15·진행 14/15)은 직전 단계 헤딩에 병기
- 마지막 실제 상태 대조: **2026-08-28 15:30 KST (검토 PC)** — local main `5f4bd8ecdfc37b7c3dd808e4e19da85517a578f8` = origin/main; worktree dirty는 storyline runner·테스트·현행 문서 배치뿐. `/health` memory/knowledge enabled·ready 및 knowledge `144/287`(검토 PC DB와 일치; Codex PC는 153/296), model digest 명시 핀 verified, 운영 관련 실험 flag OFF, proxy 11435·latency 8892·Ollama 11434 listening(TTS/GPT-SoVITS/STT 미기동 — storyline 러너는 텍스트 전용이라 측정에 무관), `run_vod_storyline` PID 0. run-83은 약 12/32 중단이며 terminal report가 없다.
...
- [B] M8-7 Stage 3: Mi:dm 교정 파인튜닝 — Small Model Learnability Gap 교정(arXiv 2502.12143: 작은 teacher·짧은 예시·Mix Distillation), 결함 목록=평가셋, Mi:dm 베이스 유지(모델 교체 아님). **차단 원인: 검토 PC에 CUDA 없음 — 물리적 실행 불가. 재개 조건: GPU PC 확보** (Stage 1·2 소진으로 다른 선행 조건은 이미 충족)
- [B] M8-8 Stage 4: 모델 축 진단(구 S5 확장) — qwen3:8b/4b think-off·RP 튜닝 adelie-7B·동급 kanana-nano-2.1b를 같은 하네스·같은 32턴으로, 수동 검토에 **한국어 자연스러움 축 명시**. 차단 원인: 사용자 지시 "모델 변경은 가장 마지막"(Mi:dm 한국어 능력 유지); 재개 조건: Stage 1~3 전부 소진 후에도 3연속 미달
- [ ] M8-9 아키텍처 반전 설계 검토 — 결정론 스파인(beat를 말할 수 있는 대사로 재저작)+모델은 시청자 리액션만. Stage 진행과 병행 가능한 설계 문서 작업, 실행은 별도 결정
