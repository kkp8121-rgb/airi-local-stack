...
- 현재 하는 일: run 05 사람 채점과 다음 축(D/E/F) 결정 대기 (M6-9/M6-10) — 실행 중인 작업은 없다
...
- 현재 진행: **M6 8/10 완료 — A+B 구현·run 05 재생 종료, 사람 채점과 다음 축 결정 대기** — 2026-08-26 12:35 KST 기준
...
- 다음 작업: 사용자가 run 05 `rating-sheet.html` 채점 → 기준선 대비 판정(계약 §4) → M6-10 결정 → run 06
- 다음 결정: 사용자 실제 세션 캡처 시점(M6-6), 기준선 뒤 첫 개선 후보 선택(M6-8). 운영 채택·GPU
  학습·push는 계속 별도 승인이다.
- M6 정상 완료율: **8/10 (80.0%)** — `[x] / 활성 항목 전체`
- M6 처리 종료율: **8/10 (80.0%)** — `([x] + [F]) / 활성 항목 전체`
- M6 진행 지수: **8.5/10 (85.0%)** — `([x] + [F] + 0.5×[P] + 0.5×[~]) / 활성 항목 전체`
- 전체 로드맵 정상 완료율: **59/87 (67.8%)**
- 전체 로드맵 처리 종료율: **65/87 (74.7%)**
- 전체 로드맵 진행 지수: **68/87 (78.2%)** — 96개 행 중 `[S]` 8개와 `[N/A]` 3개를 분모에서 제외; 2026-08-26 11:45 KST 기준
...
- [D] 남은 결함(모델 고유 되묻기·에코 붕괴)의 다음 축 결정 (M6-10) — D 되먹임 위생 일반화 / E 모델 축 probe(qwen3:8b 평가
  전용 재생) / F 프롬프트 규칙; 파인튜닝은 E 결과 뒤에만 재검토
...
- 활성 항목 전체는 `[S]`와 `[N/A]`를 제외한 행이다. `[F]`는 정상 완료에는 포함하지 않고
  처리 종료에는 포함한다. 위 비율은 현재 M6의 10개 분리 행을 기준으로 계산했다.
- 전체 로드맵의 과거 raw checkbox 55행을 코드·테스트·receipt·commit·사용자 결정과 대조했다.
  ...
  M5 11행과 M6 10행을 더해 전체 98행이 됐다(2026-08-26 v4). 상태별 개수는 `[x]` 59, `[~]` 0, `[Q]` 1, `[ ]` 0, `[P]` 6,
