...
- 현재 하는 일: 사용자 실제 세션 캡처 대기 (M6-6) — 코드·도구·문서는 준비됐고 실행 중인 작업은 없다
...
- 현재 진행: **M6 5/8 완료 — commit 3건(`a9583c3`·`cdbb6eb`·`776b462`, 로컬), 로드맵 v4·문서 다이어트·
  grounding 수리·채점 도구 종료; 실제 세션 캡처는 사용자 몫** — 2026-08-26 11:53 KST 기준
...
- 다음 작업: 사용자 실제 세션 캡처 → export → 채점 시트 → 요약(M6-6/7). 저장소 push는 별도 승인
- 다음 결정: 사용자 실제 세션 캡처 시점(M6-6), 기준선 뒤 첫 개선 후보 선택(M6-8). 운영 채택·GPU
  학습·push는 계속 별도 승인이다.
- M6 정상 완료율: **5/8 (62.5%)** — `[x] / 활성 항목 전체`
- M6 처리 종료율: **5/8 (62.5%)** — `([x] + [F]) / 활성 항목 전체`
- M6 진행 지수: **5/8 (62.5%)** — `([x] + [F] + 0.5×[P] + 0.5×[~]) / 활성 항목 전체`
- 전체 로드맵 정상 완료율: **56/85 (65.9%)**
- 전체 로드맵 처리 종료율: **62/85 (72.9%)**
- 전체 로드맵 진행 지수: **64.5/85 (75.9%)** — 96개 행 중 `[S]` 8개와 `[N/A]` 3개를 분모에서 제외; 2026-08-26 11:45 KST 기준
- 마지막 실제 상태 대조: **2026-08-26 11:53 KST** — HEAD local main `776b462`(origin/main `d7c6283`보다 3 ahead,
  push 미수행), proxy/runtime unittest 414 OK, human_review 16 passed, matrix·campaign·서비스 PID 0,
  파인튜닝 후보 채택 0
...
- [D] 사용자 실제 세션 캡처 — 비공개 테스트 방송 또는 직접 채팅 50~100턴 (M6-6) — 사용자 실행 시점 결정
- [ ] 사람 채점 실행·기준선 확정 (M6-7) — M6-6 뒤, 가능하면 채점자 2명
...
- 전체 로드맵의 과거 raw checkbox 55행을 코드·테스트·receipt·commit·사용자 결정과 대조했다.
  ...
  M5 11행과 M6 8행을 더해 전체 96행이 됐다(2026-08-26 v4). 상태별 개수는 `[x]` 56, `[~]` 0, `[Q]` 1, `[ ]` 1, `[P]` 5,
  `[B]` 9, `[D]` 7, `[F]` 6, `[S]` 8, `[N/A]` 3이며 `[?]`는 없다.
