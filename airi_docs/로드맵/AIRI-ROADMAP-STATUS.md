...
- [ ] P1-1 턴 브리핑 조립기 (디렉터 확장 — 결정론, 상수는 테이블화)
- [ ] P1-2 viewer_memory 배선 (I2a 기반은 테스트 green — 주입만 미배선)
- [ ] P1-3 시뮬레이션 재실측 — §1 "시청자 사실 활용" 지표 신설 포함
- [ ] P1-4 **C안: 선반응 ACK "응!" 제거** — 사용자 확정 2026-08-19.
  env `AIRI_IMMEDIATE_ACK: audible(기본)|marker|off` 신설, 런처 기본값
  (marker=표정만 vs off)은 구현 착수 시 확인. 근거: "응! 응," 이중
  19/48 + 매 턴 동일 오프닝 48/48
...
- [ ] P2-1 thank 렌더러 **운영 배선** — B4b 어댑터 →
  `callout_context`. 렌더러 자체는 완료(2026-08-18, default-inert,
  테스트 14). 배선·비공개 리허설·운영 ON은 사용자 승인 경유.
  근거: 생일 후원 반사 4/4 arm + 호명 0/5(닉네임 줘도 0)
- [ ] P2-2 여론 집계 발화 — 같은 tag N건 이상이면 디렉터가 집계 문장
  생산("지금 노래 얘기 많네" 류). **문구 풀은 사용자 승인 대상**
- [ ] P2-3 "기억나?" 가드 — 회상 근거 없으면 "기억해" 단정 금지
  (거짓 기억 주장 차단 — 안전 성격). 방식 검토 후 제안
...
| # | 항목 | 상태 |
|---|---|---|
| 0 | ACK 제거 C안 | **확정** — 기본값(marker/off)만 P1-4 착수 시 확인 |
| 1 | thank 렌더러 운영 배선 (P2-1) | 대기 |
| 2 | 여론 집계 발화 문구 (P2-2) | 대기 |
| 3 | "기억나?" 가드 (P2-3) | 대기 |
| 4 | B4c 파라미터 폼 9항목 | 회신 대기 — 통합 폼 §C (`진행예정/AIRI-DECISION-FORM-2026-08-19.html`, 구 폼 대체) |
| 5 | 침묵 폴백 풀 운영 ON | 문구 승인됨·env 기본 OFF — 런처 채택 여부 |
| 6 | 계약 v3+게이트 운영 ON | #4와 묶임 |
| 7 | 팬덤명 | 유보 ("시청자들" 일반 호칭) |
| 8 | T-05 목소리 샘플 청취 | 대기 |
| 9 | DeepL 키 회전 (보안, talkain) | 권고 전달됨 |
| 10 | §1 목표치 확정 | 제안값 상태 |
