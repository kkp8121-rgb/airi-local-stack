...
> 산출물·PID·receipt를 대조해 갱신한다. 2026-08-26부터 채택 게이트는 **실제 대화 사람 채점**
> (`진행중/AIRI-REAL-DIALOGUE-HUMAN-EVAL-CONTRACT-2026-08-26.md`) 하나뿐이며 합성 fixture blind 매트릭스는 회귀 도구다.

- 현재 목표: 실제 대화 기반 평가 체계 수립과 방송 배관 수리 (M6, `goal_status=active`)
- 현재 하는 일: 사용자 최종 채점·첫 개선 후보 결정 대기 (M6-7/M6-8) — 실행 중인 작업은 없다
- 왜 하는가: 사용자가 5라운드 합성 평가를 보고 공회전으로 판정했다(실태:
  `진행중/AIRI-REALITY-CHECK-AND-FINETUNE-REVIEW-2026-08-26.md`). 앞으로는 실제 방송/채팅 로그를 사람이 채점한 결과로만
  채택을 결정하고, 그 전에 실제 방송에서 바로 드러날 배관 결함을 먼저 고친다.
- 현재 진행: **M6 6/8 완료 — 실제 채팅 수집·재생(run 04)까지 종료, 사람 최종 채점과 첫 개선 후보 결정 대기** — 2026-08-26 12:35 KST 기준
- 정지 사유: 없음. matrix·campaign·서비스·GPU 학습은 실행 중이 아니다. 파인튜닝은 중단(재개 조건은
  실태 문서 §4.2).
- 완료 조건: M6-6은 사용자가 실제 세션(테스트 방송 또는 직접 채팅) 50~100턴을 memory ON 스택으로
  남기고 export JSONL을 저장소 밖에 만들 때, M6-7은 채점 JSON 요약으로 기준선이 확정될 때 닫는다.
  M6 전체는 기준선 확정과 첫 개선 후보 결정(M6-8)으로 닫는다.
- 다음 작업: 사용자가 `rating-sheet.html`로 최종 채점 → `summarize_ratings.py`로 기준선 확정(M6-7) → 첫 개선 후보(M6-8)
- 다음 결정: 사용자 실제 세션 캡처 시점(M6-6), 기준선 뒤 첫 개선 후보 선택(M6-8). 운영 채택·GPU
  학습·push는 계속 별도 승인이다.
- M6 정상 완료율: **6/8 (75.0%)** — `[x] / 활성 항목 전체`
- M6 처리 종료율: **6/8 (75.0%)** — `([x] + [F]) / 활성 항목 전체`
- M6 진행 지수: **6.5/8 (81.3%)** — `([x] + [F] + 0.5×[P] + 0.5×[~]) / 활성 항목 전체`
- 전체 로드맵 정상 완료율: **57/85 (67.1%)**
- 전체 로드맵 처리 종료율: **63/85 (74.1%)**
- 전체 로드맵 진행 지수: **66/85 (77.6%)** — 96개 행 중 `[S]` 8개와 `[N/A]` 3개를 분모에서 제외; 2026-08-26 11:45 KST 기준
- 마지막 실제 상태 대조: **2026-08-26 12:35 KST** — run 04 report 99+1턴·service_error 0, 스택 stop·listener 0, local HEAD
  `f408eb7`(origin/main `39c29bf`+2, push 미승인), broadcast_sim+human_review 269 passed, 파인튜닝 후보 채택 0

### 현재 주요 작업 단계(M6) 체크리스트
...
- [P] 사람 채점 실행·기준선 확정 (M6-7) — 2026-08-26 12:35 Claude 1차 참고 채점만 완료(방송다움 1.89·맥락 1.85·반응 1.99·
  말투 4.03·사실성 3.99, 무의미 대꾸 44%); 사용자 최종 채점(rating-sheet.html) 대기
- [D] 기준선 뒤 첫 개선 후보 선택 — 재료 공급/결정론 계층/프롬프트/모델 교체 (M6-8)
...
- 활성 항목 전체는 `[S]`와 `[N/A]`를 제외한 행이다. `[F]`는 정상 완료에는 포함하지 않고
  처리 종료에는 포함한다. 위 비율은 현재 M6의 8개 분리 행을 기준으로 계산했다.
- 전체 로드맵의 과거 raw checkbox 55행을 코드·테스트·receipt·commit·사용자 결정과 대조했다.
  ...
  M5 11행과 M6 8행을 더해 전체 96행이 됐다(2026-08-26 v4). 상태별 개수는 `[x]` 57, `[~]` 0, `[Q]` 1, `[ ]` 0, `[P]` 6,
  `[B]` 9, `[D]` 6, `[F]` 6, `[S]` 8, `[N/A]` 3이며 `[?]`는 없다.
...
- 주요 작업 단계(M): 이 대시보드에서 `M`은 Milestone이다. 현재 M6의 범위는 “실제 대화 사람 채점
  평가 체계 + 방송 배관 수리”이며, 하단의 과거 M1~M5 분류는 전체 로드맵 감사 때 현재 상태 표준으로
  재분류한다.
...
2. **파인튜닝은 중단**한다. 재개 조건(실제 코퍼스·채점 세트 동결·결정론으로 못 닫는 행동 실측·
   일반 능력 게이트)을 모두 채울 때만 검토한다. 기존 어댑터는 보관·채택 금지.
...
> 2026-08-26 v4: 위 broadcast_sim 지표는 회귀 가드로만 유지한다. 주 지표는 실제 대화 사람 채점
> 5축 평균과 플래그율이며, 첫 기준선은 M6-7에서 확정한다.
...
- [F] **P2-6b 총평 반영 학습·품질 트랙:** 사용자가 파일럿의 길이·후원
  ...
  운영 채택하지 않는다.
- [B] P2-6c 후속 행동 데이터·학습 라운드 — 차단 원인: 2026-08-26 파인튜닝 중단 결정(실태 문서 §4.2);
  재개 조건: 실제 대화 코퍼스·사람 채점 세트 동결·결정론으로 못 닫는 행동 실측·일반 능력 게이트 충족
...
- [B] P3-T2d 추가 인간 검수·학습 라운드 — 차단 원인: 2026-08-26 파인튜닝 중단 결정; 재개 조건: 실태 문서
  §4.2의 4개 조건 전부 충족
...
- [D] P3-M1 코덱스 GPU 재실측에 4B급 공존 옵션 포함 — 사용자 방향 결정 필요 — v4: 사람 채점 기준선(M6-7) 확정 뒤에만 검토
- [D] P3-M2 챗 모델 체급 A/B — 기존 하네스 재실행 여부를 사용자 결정 — v4: 사람 채점 기준선(M6-7) 확정 뒤에만 검토
