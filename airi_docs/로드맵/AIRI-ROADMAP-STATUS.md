...
- 현재 하는 일: S0·S1 실행 완료, 사람 채점(run 05·06b-r3) 대기 (M7-3) — 실행 중인 작업은 없다
...
- 현재 진행: **M7 2/8 완료 — S0 복제 노이즈 측정·S1 되먹임 고리 차단(복제 3회 붕괴 0/3), 사람 채점 대기** — 2026-08-26 16:55 KST 기준
- 정지 사유: 사용자 채점 대기(`20260826-replay-06b-r3/rating-sheet.html`, `20260826-replay-05/rating-sheet.html`). matrix·campaign·서비스·GPU 학습은
  실행 중이 아니다.
...
- 다음 작업: 사용자 채점 → `summarize_ratings --baseline`(run 04) 판정 → 3축 +0.2 이상이면 운영 flag on·하니스 runner 플래그 승인 요청 → S2(06b 구성 위 복제 3회)
- 다음 결정: run 05·06b-r3 사람 채점, 추가 commit push 승인, (채점 뒤) 운영 `AIRI_FEEDBACK_HYGIENE=on` 승인. GPU 학습·파인튜닝(2026-09-09까지 금지)은 별도.
- M7 정상 완료율: **2/8 (25.0%)** — `[x] / 활성 항목 전체`
- M7 처리 종료율: **2/8 (25.0%)** — `([x] + [F]) / 활성 항목 전체`
- M7 진행 지수: **2/8 (25.0%)** — `([x] + [F] + 0.5×[P] + 0.5×[~]) / 활성 항목 전체`
- 전체 로드맵 정상 완료율: **62/94 (66.0%)**
- 전체 로드맵 처리 종료율: **68/94 (72.3%)**
- 전체 로드맵 진행 지수: **70.5/94 (75.0%)** — 107개 행 중 `[S]` 10개와 `[N/A]` 3개를 분모에서 제외; 2026-08-26 17:25 KST 기준
- 마지막 실제 상태 대조: **2026-08-26 17:25 KST** — local HEAD `f748cae`(origin/main `e6e745a`+2, push 미승인), 서비스·matrix PID 0,
  13 run 산출물 `D:\AIRI-Models\airi-human-eval\20260826-replay-05r*/06a-r*/06j-r*/06b-r*`, 채점 시트 06b-r3·06a-r1·05
...
- [x] S1 proxy 되먹임 고리 차단 flag 구현·복제 측정 (M7-2) — 2026-08-26 17:21 `098c887` `AIRI_FEEDBACK_HYGIENE` off/journal/on(기본 off byte 동일,
  테스트 10건, proxy 394 OK, health 노출); 복제 3회: on 붕괴 1/3, journal 2/3(무효), on+브리핑 자기 에코 차단 **0/3**(off 3/4) — 완전 에코 0,
  "?" 종결 7/17/16. 운영 기본값은 여전히 off; 켜는 것은 M7-3 사람 채점 뒤 별도 승인
- [D] 사람 채점 — run 05(A+B 대조군)와 `06b-r3`(S1 최종 arm, 사전 등록 규칙 "?" 종결 중앙값) (M7-3) — 두 `rating-sheet.html` 준비됨,
  사용자 채점 대기; 판정은 기준선(run 04) 대비 3축 합성 `summarize --baseline`
- [ ] S2 오프너 중복 재샘플 — 초안 첫 2어절이 직전과 같고 둘 다 '?'면 1회 재샘플 (M7-4) — S1 뒤
- [ ] S3 예시 4쌍 — 규칙 문장 대신 반말 U/A 예시를 live-broadcast 요청에만 flag 삽입 (M7-5) — S2 뒤
- [ ] S4 픽업 스킵·배치 — 내용 토큰 임계와 합창 batched_chat (M7-6) — S3 뒤
- [ ] S5 모델 축 1회 진단 — qwen3:8b, TTS 미기동·전량 GPU·think off·같은 샘플링·seed 2개, 채택 아님 (M7-7) — 06b 구성 위에서
- [B] 파인튜닝 재진입 판정 (M7-8) — 차단 원인: 타계책에 따라 2026-09-09까지 파인튜닝 금지; 재개 조건: S1~S5 뒤에도
  3축 합성 < 3.0이고 S5에서 큰 모델이 +0.6 이상(용량 병목 실측)이며 실제 채팅 입력 + 사람 4점 이상 검증 답변 ≥ 300턴
...
- 활성 항목 전체는 `[S]`와 `[N/A]`를 제외한 행이다. `[F]`는 정상 완료에는 포함하지 않고
  처리 종료에는 포함한다. 위 비율은 현재 M7의 8개 분리 행을 기준으로 계산했다.
- 전체 로드맵의 과거 raw checkbox 55행을 코드·테스트·receipt·commit·사용자 결정과 대조했다.
  ...
  M5 11행·M6 11행·M7 8행을 더해 전체 107행이 됐다(2026-08-26 v4·타계책). 상태별 개수는 `[x]` 62, `[~]` 0, `[Q]` 1, `[ ]` 4, `[P]` 5,
  `[B]` 10, `[D]` 6, `[F]` 6, `[S]` 10, `[N/A]` 3이며 `[?]`는 없다.
...
| 19 | (2026-08-26) 운영 `AIRI_FEEDBACK_HYGIENE=on` + 하니스 브리핑 자기 에코 차단 채택 | **대기** — M7-3 사람 채점이 3축 +0.2 이상일 때 요청 |
