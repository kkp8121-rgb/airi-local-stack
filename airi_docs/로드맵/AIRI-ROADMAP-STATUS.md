...
- M8 처리 종료율: **7/11 (63.6%)** — `([x] + [F]) / 활성 항목 전체`
- M8 진행 지수: **7.5/11 (68.2%)** — `([x] + [F] + 0.5×[P] + 0.5×[~]) / 활성 항목 전체`
- 전체 로드맵 정상 완료율: **74/112 (66.1%)**
- 전체 로드맵 처리 종료율: **86/112 (76.8%)**
- 전체 로드맵 진행 지수: **90/112 (80.4%)** — 127개 행 중 `[S]` 12개와 `[N/A]` 3개를 분모에서 제외; 2026-09-01 21:35 KST 기준. 직전 M7 요약(정상 11/15·처리 13/15·진행 14/15)은 직전 단계 헤딩에 병기
- 마지막 실제 상태 대조: **2026-09-01 21:36 KST (GPU PC = Codex PC)** — local main `a648aa5f3659a98ff34116a902808b6c58b6010c` = origin/main. CUDA 실측 가용(RTX 3060 Ti 8 GiB, torch 2.7.0+cu128). `/health` memory/knowledge enabled·ready, knowledge `153/296`(이 PC DB와 일치; 검토 PC는 144/287), 평가 태그 `midm-airi-s3:eval-r3` digest 명시 핀 verified, 운영 태그 `midm-airi:2.0-mini`와 그 핀은 불변. proxy 11435·latency 8892·Ollama 11434 listening. dirty worktree는 storyline 러너·테스트·신규 스파인 사이드카와 현행 문서 배치이며 commit/push 미수행. M8-9 재현 회차는 외부 중단 2회(22/32, 13/32)로 terminal report가 없어 증거 가치 0이다.
