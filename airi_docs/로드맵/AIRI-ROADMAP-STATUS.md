...
- 현재 진행: **M7 2/8 완료 — S0 복제 노이즈 측정·S1 되먹임 차단(템플릿 고정 0/3, 문구 고정 2/3, 참고 채점 개선 없음), 사람 채점 대기** — 2026-08-26 16:55 KST 기준
...
- 다음 작업: 사용자 채점 → `summarize_ratings --baseline`(run 04) 판정 → (참고 채점대로 개선 없으면) S6 입력 정합(자기완결 저챗 구간 선별
  또는 스트리머 STT 발화를 AIRI 직전 발화로 시간 정렬 공급)을 S2~S5보다 먼저 사용자에게 제안
- 다음 결정: run 05·06b-r3 사람 채점, 추가 commit push 승인, (채점 뒤) 운영 `AIRI_FEEDBACK_HYGIENE=on` 승인. GPU 학습·파인튜닝(2026-09-09까지 금지)은 별도.
- M7 정상 완료율: **2/9 (22.2%)** — `[x] / 활성 항목 전체`
- M7 처리 종료율: **2/9 (22.2%)** — `([x] + [F]) / 활성 항목 전체`
- M7 진행 지수: **2/9 (22.2%)** — `([x] + [F] + 0.5×[P] + 0.5×[~]) / 활성 항목 전체`
- 전체 로드맵 정상 완료율: **62/95 (65.3%)**
- 전체 로드맵 처리 종료율: **68/95 (71.6%)**
- 전체 로드맵 진행 지수: **70.5/95 (74.2%)** — 108개 행 중 `[S]` 10개와 `[N/A]` 3개를 분모에서 제외; 2026-08-26 17:25 KST 기준
- 마지막 실제 상태 대조: **2026-08-26 17:32 KST** — local HEAD `29181ba`(origin/main `e6e745a`+3, push 미승인), 서비스·matrix PID 0,
...
- [ ] S6 입력 정합 — 자기완결 저챗 구간 선별 또는 VOD 스트리머 발화(로컬 STT) 시간 정렬 공급, 사용자 결정 필요 (M7-9) — S1 결과로 S2~S5보다 앞섬
- [ ] S2 오프너 중복 재샘플 — 초안 첫 2어절이 직전과 같고 둘 다 '?'면 1회 재샘플 (M7-4) — S6 뒤
...
- 활성 항목 전체는 `[S]`와 `[N/A]`를 제외한 행이다. `[F]`는 정상 완료에는 포함하지 않고
  처리 종료에는 포함한다. 위 비율은 현재 M7의 9개 분리 행을 기준으로 계산했다.
- 전체 로드맵의 과거 raw checkbox 55행을 코드·테스트·receipt·commit·사용자 결정과 대조했다.
  ...
  M5 11행·M6 11행·M7 9행을 더해 전체 108행이 됐다(2026-08-26 v4·타계책). 상태별 개수는 `[x]` 62, `[~]` 0, `[Q]` 1, `[ ]` 5, `[P]` 5,
...
| 19 | (2026-08-26) 운영 `AIRI_FEEDBACK_HYGIENE=on` 채택 | **보류** — 참고 채점 3축 1.59 < 기준선 1.76, 사람 채점으로 뒤집히지 않는 한 요청하지 않음 |
