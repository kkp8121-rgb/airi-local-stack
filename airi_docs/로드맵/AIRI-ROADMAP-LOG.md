...
- 2026-09-01 23:30 KST **M8-9 스파인 + r4 교정 학습 — ⑥ 닫힘, ④ 재실패, 수동 5/7 고정.** ①M8-9 v0(스파인이 줄거리 두 슬롯을 말하고 모델은 리액션 한 문장)에서 수동 5/7을 얻고 재현까지 확인했다. 지어내기·beat 모순·주어 뒤집힘이 구조적으로 0이 됐고 회차는 4분으로 줄었다. ②잔여 2항목(⑥경험 도용·④반복)은 전부 모델이 쓰는 한 문장 안에 있었고, cue 레버 3종을 시도해 전부 측정으로 폐기했다 — 스파인 텍스트를 보여주면 완전 복사 9턴(최대 50자), 낭독 거부를 걸면 패러프레이즈로 이동, 텍스트 없이 지시만 주면 모순이 2→4턴으로 증가. **cue에 재료든 지시든 더하면 이 2.3B는 나빠진다**가 M8-3·M8-4에 이은 3번째 재현이라 코드 주석과 테스트로 고정했다. ③그래서 학습으로 옮겨 r4(`viewer_reaction` 타깃만 논평체로 재저작, 나머지 타깃 byte-identical, 192행 유지)를 돌렸고, 세션 재시작 대비로 정식 pause 계약(`pause.request.json`)을 써서 checkpoint-00000007에 안전 정지 후 `--resume-from-checkpoint`로 재개해 완주했다. ④결과: **⑥경험 도용이 2→0으로 닫혔다** — cue로 세 번 못 닫은 것을 학습 한 회차가 닫았다. 그러나 ④반복이 새 tic으로 재실패했고 (`제일` 10/32턴, `제일 먼저` 5턴) **이는 v1 동의형 오프너·v2 감정 라벨에 이어 내가 심은 세 번째 공식**이다. 총점은 5/7 그대로이고 통과 항목만 회전했다. ⑤판단: tic만 또 바꾸는 건 네 번째 두더지잡기이므로 중단한다. 원인은 32슬롯 코퍼스가 작아 저자의 어휘 습관이 각인되는 것이고, dev loss가 매 회차 epoch 1에서 최저를 찍는 것과 같은 뿌리다. 남은 선택지는 (a)다른 storyline 저작으로 교정 슬롯 종류를 늘리거나 (b)M8-9 v0을 5/7 도달점으로 고정하고 잔여 2항목을 저작·구조 문제로 재분류하는 것이며 사용자 결정 대상이다. 검증: broadcast_sim 269 passed/1 skipped, py_compile·diff-check clean, 문서 계약 PASS. 연속 통과 0/3, adoption false, 운영 태그 불변, commit/push 미수행. 부수 기록: Ollama 강제 종료가 고아 llama-server를 남겨 RAM 11.6 GiB를 잠식하고 회차 2건을 죽인 사고를 규명·정리했고, 이후로는 `ollama stop`으로 모델을 내린다.

- 2026-09-01 21:35 KST **Stage 3 학습 실행·소진 → M8-9 아키텍처 반전 실행, 수동 5/7 도달.** GPU PC(Codex PC)에서 CUDA 실측 후 M8-7의 유일한 차단 원인이 해소돼 이 프로젝트 최초로 학습이 지표를 개선했다. ①기준선: run-89 구성 그대로 32턴 완주(10.2분, 바닥 전부 통과, 수동 실패) — 이 PC의 before 기준점. run-89·run-92 원본은 이 PC에 없어(검토 PC 보관) 기록된 결함 분류 + 로컬 기준선을 근거로 삼았고 새 합성 시나리오는 만들지 않았다. ②교정 데이터: 96슬롯을 런타임과 동일한 대화 형태로 저작, 러너 자신의 `parse_slot_output`이 내 초안 5건을 거부해 수정, 슬롯당 2행(attempt 1 cue + retry cue는 실제 호출의 16분의 15)으로 192행. T06·T22·T14·T30을 dev/test로 사전 홀드아웃. 부수 실측으로 cue 절단 가설 배제(448~779자 vs 예산 3900자). ③학습 r1은 `--max-steps`가 microstep임을 놓쳐 0.375 epoch로 폐기, r2·r3 모두 dev loss가 epoch 1 최저 후 상승해 코퍼스 상한을 실측. merge→GGUF Q4_K_M→평가 전용 태그로 가되 `ollama create`가 운영 template·params blob을 재사용해 **가중치만 다른 단일 변수 조건을 검증**했다(운영 태그 불변). ④결과: 정렬 축 전면 개선(조언 8→0·존댓말 3→0·질문 20→0·예시 오염 4→0), 홀드아웃 동일 개선, 암기 exact 0/184 — 전이 확정. 능력 축은 불변이고 데이터 설계 v1/v2가 서로를 상쇄해 도달 가능한 최선이 통과선 아래임이 확정돼 Stage 3 종료. ⑤M8-9 착수: 스파인 사이드카 32턴 + `--rewrite-format spine` 구현(로더가 스파인도 같은 슬롯 계약으로 fail-closed 검증), run-01에서 **수동 5/7 통과**로 프로젝트 최고 기록. cue 레버 3종은 전부 측정으로 폐기하고 낭독 거부만 유지. 재현 회차는 외부 중단 2회로 미완이라 3연속 판정은 착수 전. 검증: broadcast_sim 269 passed/1 skipped, py_compile·diff-check clean, 문서 계약 PASS. adoption false·운영 flag OFF·운영 태그 불변·commit/push 미수행.

- 2026-09-01 15:40 KST **M8 Stage 2 완료·소진 → GPU PC 인계 문서 작성.** Stage 2를 **프록시 무변경**으로 구현했다: Ollama가 이미 번들한 `llama-server.exe`가 `--logit-bias`·`--grammar`를 지원함을 확인하고, 신규 평가 전용 `ollama-proxy/eval/llama_server_shim.py`(+18 오프라인 테스트)를 11434에 두어 프록시↔llama-server(11500)를 중계했다. 차단 집합은 추측이 아니라 실측으로 만들었다 — 표준 표면형 14개는 실패(모델이 ` 좋습니다`=6623, `안녕하세요`=7602 같은 공백 접두 병합 토큰으로 우회)했고, GGUF `tokenizer.ggml.tokens` 131,392개를 덤프해 GPT-2 byte 역변환으로 디코딩한 뒤 **534개(질문 234·존댓말 300)**를 전수 열거했으며 서사 필수어와 충돌 0을 확인했다. 착수 중 단일 변수 규율을 깨는 결함 2건을 잡았다: ①프록시가 주입하는 `top_p`·`repeat_penalty`를 shim이 버려 첫 시도(run-91)를 0.7분에 폐기하고 red/green으로 전달을 계약화 ②llama-server가 구 14토큰 세트로 떠 있어 534개로 재기동. **run-92 결과**(SHA256 `8dd93c3a…cd54`): 사전 등록 보장은 지켜져 `?`가 11턴→**0턴**으로 구조적 제거됐으나 수동 실패 — 존댓말 미제거(1→1, `꾸셨`의 시/셨 어간 누락), 의문 **행위**는 생존해 종결부 없는 run-on·말줄임(0→3턴)으로 변형, 조언 2→4턴(T07은 위장 손상까지 지어냄), 자기 경험·역할 전도·반복 불변. **결론: 서빙 계층을 완전히 통제해도 형식만 닫히고 의미는 닫히지 않는다.** Stage 1·2 소진으로 남은 단계는 Stage 3(Mi:dm 교정 파인튜닝)뿐이고 검토 PC에 CUDA가 없어 **GPU PC가 게이팅 자원**이 됐다. 신규 `진행중/AIRI-GPU-PC-HANDOFF-2026-09-01-STAGE3.md`를 단일 진입점으로 작성하고 WORKING/NEXT/INDEX를 동기화했다. 유효 품질 통과 0/3, adoption false, 운영 flag OFF, 미커밋 12건은 승인된 push 대기.

- 2026-08-30 13:30 KST **저장공간 정리 최종 receipt.** 실제 설정·PID·manifest와 대용량 경로를 대조한 뒤 활성/원본/개인/평가 자산을 제외하고 3,992개 대상을 영구 삭제했다. 실패 0, 명령 직접 측정 회수량은 C: 124.164 GiB + D: 101.256 GiB = 225.420 GiB, 최종 여유 공간은 C: 170.926 GiB·D: 327.343 GiB다. 활성 Mi:dm manifest SHA와 4 blob, KURE-v1·STT·GPT-SoVITS·참조 음성·DB, D: 원본·venv·실제 채팅·사람 평가·517개 report/manifest 계열 파일을 확인했다. 지정 weight/checkpoint/orphan/cache/log/tmp 잔여는 0이다. work-continuity/dashboard, source archive, entrypoint, TTS cache readiness, ASAR, sender 32, runtime fence 9+2 skip, broadcast rehearsal 85, chat ingress 47, director 24, latency 5는 통과했다. 종합 checkpoint는 독립 결함 세 종류(durability terminal timing 2회 재현, CI matrix 누락 3건, input-safety policy hash mismatch 5/16)로 green이 아니며 이 유지보수 범위에서는 수정하지 않았다. 구현·서비스·운영 모델·DB·개인 데이터·commit/push 변경 없음.

- 2026-08-28 16:10 KST **M8(스택·기술 한계 돌파) 개설 — 전 게이트 해제·모델 축 최후순위.** 사용자 결정 3건 반영: ①"모든 금지를 해제시키고 할 수 있는 건 다 해보자"(실험 게이트 전면 해제; 원문/키 반입 금지·push 매회 승인·운영 채택 별도 결정은 유지, 검토 PC는 CUDA 부재로 파인튜닝 물리 불가) ②"개선 로드맵·체크리스트 먼저 작성" ③"모델 변경은 가장 마지막 — Mi:dm 선정 사유에 한국어 능력". ROADMAP-STATUS를 재구조화: M8 11행 신설(Stage 1a CRANE식 출력 격리 → 1b 스토리 캐릭터 프레이밍 → 1c 정제 예시 1~2 → 1d depth injection, 최선 조합 재현 2회로 3연속 판정 → Stage 2 llama-server logit_bias·최소 GBNF[B] → Stage 3 Mi:dm 교정 파인튜닝[B, GPU PC] → Stage 4 모델 축 진단[B, 최후순위] + 아키텍처 반전 설계·품질 게이트 재정의[D]), 관측 프로토콜을 M8 서두에 명문화(자동=바닥만·수동 7항목=판정·검증기 적층 재개 금지), M7은 정상 11/15·처리 13/15로 종결해 직전 단계로 이동(구 S5·M7-8은 M8-8/M8-7로 [S] 대체), 결정 큐 24 **결정**·25 신설, 전체 127행 재계산(72/112·80/112·83.5/112). 대시보드 계약 테스트 PASS. 실행은 M8-1부터이며 아직 시작 전.

- 2026-08-28 15:42 KST **대안 기법 리서치 완료(사용자 Goal).** 웹 리서치 subagent 2축(논문/GitHub·HF) 종합 → `진행중/AIRI-METHOD-RESEARCH-2026-08-28.md`. 수렴 결론: ①Ollama가 GBNF·logit_bias·control vector를 전부 막고 있고 같은 GGUF가 llama-server에서는 셋 다 열림 ②소진한 best-of-N+자체검증기 트랙은 ≤13B 약한 verifier·reward-ambiguous 롤플레이 근거로 논문에서도 반증(2404.17140·2310.01798·2508.09074) ③문법 완전 강제는 역효과(2605.02363), 검증된 대안은 CRANE 자유서술+출력 격리(2502.09061, 1.5B 실증) ④few-shot은 "지시문+정제 예시 1~2개"가 정답 방향(2402.09954·2303.08119), 기대치는 낮게(2409.15790) ⑤"스토리 캐릭터" 프레이밍이 어시스턴트 잔재 없이 더 깨끗(2608.07852) ⑥한국어 RP 소형 모델 생태계 공백(최근접 adelie 7B; 동급 kanana-nano-2.1b; KREW korean-role-playing 데이터셋은 즉시 few-shot 소스) ⑦과거 파인튜닝 6라운드 전패는 Small Model Learnability Gap(2502.12143)으로 설명 가능 — 재시도 시 작은 teacher·짧은 예시·Mix Distillation. 권고 경로 A(단기 콤보 1회차)→C(S5 해제 판정)→B/D 분기를 결정 큐 24에 등재, 실행은 사용자 선택 대기. 저장소 기본값·회차 시작 없음.

- 2026-08-28 15:30 KST **검토 PC 재개 → run-81/82 완주 → validator 튜닝 트랙 사용자 중단·한계 실증 고정.** ① 재개 대조: `git pull --ff-only` `53befcb→5f4bd8e`(기대 SHA 일치), 검토 PC 지식 DB는 153이 아니라 **0/0 실측** → 저장소 배치 3종을 runtime 절대경로로 game→meme→culture 적재(`41/0/0`·`55/0/0`·`48/0/0`, 최종 `144/287` = 예측치, probe **6/6**; Codex PC 153과의 차 9건은 그 PC에만 있던 기존 문서분). 모델 digest fail-closed는 내용 차이가 아니라 **신형 Ollama(0.32.14) manifest 직렬화 차이**(가중치 layer `31beb9a8…` byte-identical, template/params blob도 Modelfile과 byte-identical)로 규명 — 저장소 핀·Modelfile·태그 불변, 런처 지원 경로 `-ChatModelDigest`로 이 PC 실측값 명시 핀. 런처 affect 계약 거짓 실패는 **pwsh7 Int64 vs PowerShell 5.1 Int32** 차이로 규명, 5.1로 기동(코드 무변경). 이 PC는 CUDA 없음·CPU 추론(생성 16.2 tok/s 실측, 회차당 ~90분). ② run-81(`fictional-health-story-boundary`, report SHA `74d7af30…a32d3`): 자동 게이트는 emotion 하나만 실패였지만 수동 검토 실패 — 이때 **채점기 부풀림 발견**(scorer가 beat 어휘 일치로 채점 ↔ 모델이 beat 복창; 낭독 슬롯 10/96 실측). ③ 단일 원인 수정 `beat-verbatim-recitation-reject`(임계 14자, run-81 분포로 결정; red/green, 751 passed) 후 run-82(report SHA `ba4feb42…eb7d1`): 낭독 **0/96**으로 닫혔고 자동 new-event가 오히려 하락해 부풀림 진단을 실증. 수동 검토는 역할 전도(시청자에게 훈수·이야기 요구)·세계관 붕괴("촬영 특수효과")·의료 조언 잔존으로 실패. ④ 두 번째 수정 `out-of-story-voice-reject`(run-82 실측: 선택 슬롯 10/96 적중·오탐 0·풀 소진 0/96; red/green 751 passed) 적용 후 run-83 착수했으나 **사용자 결정 "지금 즉시 중단·정리"**로 약 12/32에서 중단(terminal report 없음·무근거). 유효 품질 통과 **0/3**으로 트랙 종료. 결함 분류·한계 판단(2.3B 정렬·능력 병목)·남은 선택지 4종은 신설 `진행중/AIRI-VOD-STORYLINE-CEILING-EVIDENCE-2026-08-28.md`. ⑤ 후속: 사용자 Goal로 대안 기법(few-shot·제약 디코딩·steering·RP 모델 등) 광범위 리서치 착수. 부수 기록: `SOURCE_EXPERIENCE_RE`의 `실감이 안 나`류 오탐(후속 단일 원인 후보), 세션 중 일부 checkpoint 시각이 실측 없이 기재돼 최대 ~35분 드리프트(이 배치에서 15:30 실측 기준으로 정정). adoption false·운영 flag OFF·GPU/파인튜닝·replay/pickup/S5 금지 유지, commit/push 별도 승인 대기.

- 2026-08-28 10:24 KST **Claude 인계 구현 commit receipt.** VOD storyline 하네스·고정 8막 fixture·평가 seam·테스트와 현행 문서 12개를 `b5abd7843b2198861ca885d565c831c9deb548ef`(`feat: add VOD storyline self-test harness`)로 커밋했다. 직전 검증 수치와 종합 checkpoint의 기존 CI 매트릭스 누락 2건을 숨기지 않고 인계문에 고정했다. 다음은 이 receipt를 문서 전용 commit으로 닫고, 사용자 승인 1회의 `origin/main` push 후 local/origin/remote SHA·clean worktree·health를 대조하는 것이다.

- 2026-08-28 10:21 KST **VOD storyline run-21~80 진단·Claude 인계 전환 intent/receipt.** 고정 8막·32턴 입력과 저장소 밖 회차별 report를 유지해 slotwise 3문장 생성, 후보 선택, 사건·callback·next-hook·감정 신호, 반복/중복, 출처 경험 도용, 무관 지식, 방송 시그니처를 측정했다. 자동 구조 게이트를 통과한 회차도 있었지만 수동 검토에서 의료 조언, 질문 종결, 내부 연출어, AIRI의 원방송 경험 도용이 남아 **유효 품질 통과는 0/3**이다. 최신 완주 run-78은 32/32·8/8·1536 후보·repeat/high-overlap/source-marker/irrelevant-marker 0이지만 emotion variation 실패와 수동 품질 실패다. run-79는 `--rewrite-passes 1 --rewrite-format slotwise` 누락으로 무효이며, run-80은 사용자 요청이 인계·push로 바뀌어 19/32에서 중단했고 terminal report가 없다. 게시 전 검증은 storyline 8/8·proxy 411/411·broadcast rehearsal 85/85·broadcast_sim 105/105·문서 계약·diff-check가 통과했다. 종합 checkpoint는 기준 HEAD부터 있던 CI workflow 누락 2건(`test_broadcast_examples.py`, `test_opener_resample.py`)을 정확히 보고하고 중단했다. 사용자의 명시 요청으로 live 문서와 신규 `AIRI-CLAUDE-HANDOFF-2026-08-28-VOD-STORYLINE.md`를 현행화하고 모든 repository 작업물을 한 번의 승인된 push로 게시한다. 운영·채택·GPU·파인튜닝·replay·pickup·S5는 변경하지 않는다.

- 2026-08-28 01:32 KST **M7 VOD storyline self-test run-15~20 receipt.** run-15는 run-14 설정을 재현해 repeat/high-overlap `0.000`을 확인했지만 새 사건·callback/next-hook·emotion 전수 조건은 실패했다. run-16의 응답 형식 명시, run-17의 상태 장부 후단 배치, run-18의 AIRI/VOD 주어 경계, run-19의 history-turns 0, run-20의 최종 3문장 역할 분해를 각각 한 원인씩 시험했다. 모든 회차는 32/32·8/8·32 staged authors·금지 시그니처 0·empty/service error 0이었다. 최종 run-20도 repeat/high-overlap/source-claim/irrelevant-marker `0`이지만 T1 내부 표식 노출, 한 문장/질문 종결, 새 사건·callback/next-hook·emotion 누락으로 품질 실패다. 결과는 `D:\AIRI-Models\airi-human-eval\20260828-vod-storyline-run-15`~`run-20`에 보존했다. 동일 응답 형식 문제에 대한 즉흥 수정은 중단하고 새 접근을 정한 뒤 재개한다. 채택·게이트 판정·운영 반영·GPU·파인튜닝·commit/push는 하지 않는다.

- 2026-08-28 00:25 KST **M7 VOD storyline run-06 receipt.** 저장된 STT 31.64분을 8막으로 구조화하고 각 장면에 맥락 연결형 시청자 모방 채팅 4개씩을 배치했다. run-06은 32/32 응답, 8/8 장면 순서, 32명 staged pseudonymous authors, 금지 시그니처 0, 빈 응답·서비스 오류 0으로 입력/전송 기준을 통과했다. 지식 DB는 계속 `documents=153, chunks=296`, memory/knowledge ON·ready다. 품질 `context_retention`은 사용자 직접 채점 전이므로 채택·게이트 변경·운영 반영으로 해석하지 않는다. 외부 직접 검토 HTML은 `D:\AIRI-Models\airi-human-eval\20260828-vod-storyline-run-06\vod-storyline-review.html`이며, 다음은 사용자 직접 32턴 채점이다.

- 2026-08-27 23:46 KST **M7 VOD 시뮬레이션 설계 결함 receipt.** 이전 77턴 실행은 `츕츕` 변형 3건을 입력에 포함했고, 고정 fixture의 generic beat와 pickup 직전 45초 STT만 사용했다. 따라서 VOD 시작부터 끝까지의 줄거리를 AIRI 방송으로 연출한 실행이 아니며, 기술적 exit 0과 안전 지표는 진단용으로만 보존한다. 다음은 전체 STT 시간축 장면화·장면별 채팅 동기화·AIRI 방송인 역할 계약을 먼저 검토하는 것이다. 정정 실행·자동 채점·채택·회차 선정·학습·운영 반영·S5·push는 하지 않는다.

- 2026-08-27 22:51 KST **실제 대화 스택 health receipt.** memory/knowledge가 모두 enabled·ready이고 `/health` knowledge `documents=153, chunks=296`이 적재 DB와 일치했다. 외부 memory DB `conversation_message=0`, proxy/latency/TTS listener 정상, STT OFF다. 사용자 직접 1턴을 기다린다. 이번 세션은 채택 판정 대상이 아니다.

- 2026-08-27 22:34 KST **Codex post-pull + 지식 적재 intent receipt.** `git pull --ff-only origin main`이 `62be3aa → 53befcb`로 fast-forward 완료됐다. 저장소 내 권위 자산은 지식 배치 41/55/48건, VOD chat 5,243행·transcript 500행·스트리머 응답쌍 57행이며 UTF-8 lint는 144/144 유효로 통과했다. read-only runtime 확인값은 기존 DB `documents=9, chunks=9`; 새 144건은 아직 적재하지 않았다. 다음은 세 JSONL을 runtime 안 절대경로로 복사하고 배치별 `knowledge_ingest.py --apply` 후 고정 6종 probe를 실행하는 것이다. `goal_status=paused`, adoption_authorized=false, 운영 flag OFF, replay/S5/GPU/파인튜닝/commit/push는 하지 않는다.

- 2026-08-27 22:40 KST **적재 DB 경계 결정 receipt.** 기존 runtime DB의 9건과 저장소 배치 144건은 제목 중복 0건이다. 기존 `airi-knowledge.sqlite3`를 덮어쓰지 않고 `ollama-proxy/runtime/airi-knowledge-m7-real-dialogue-20260827.sqlite3`를 새로 만들어 세 배치를 적용한다. 적용 후 count와 고정 6종 probe가 확인되기 전에는 지식 준비 완료로 승격하지 않는다.

- 2026-08-27 22:41 KST **지식 적재 완료 receipt.** 세 저장소 배치를 새 평가 DB에 순서대로 적용해 `inserted=41`, `inserted=55`, `inserted=48`을 받았다. 실제 DB count는 `documents=144, chunks=287`, 고정 6종 probe는 `6/6 (100%)`로 `--min-hit-rate 0.9`를 통과했다. 기존 runtime DB `documents=9, chunks=9`는 보존했다. 다음은 계약 §1(b) 실제 사용자↔AIRI 직접 대화 50턴+이다.

- 2026-08-27 22:43 KST **사용자 정정 receipt·실제 대화 preflight intent.** 22:41의 별도 평가 DB 선택은 폐기하고 기존 `airi-knowledge.sqlite3`를 최종 대상으로 삼는다. 기존 `documents=9, chunks=9`에서 시작해 세 배치를 하나씩 적용하며 각 `inserted/updated/duplicate`를 기록한다. 기대 상한은 153건이고 범위 밖이면 원인 규명 전 중단한다. 이후 memory ON·knowledge ON으로 스택을 기동하고 `/health`의 knowledge documents를 DB count와 대조한다. 채점 전 세션 비채택·다음 회차 적용 선언을 기록하고 사용자가 직접 채점한다. AI 채점+사용자 승인, `calibrate_ratings.py`, replay/S5/GPU/파인튜닝/운영 반영/commit/push는 하지 않는다.

- 2026-08-27 22:45 KST **지식 적재 완료 receipt.** 기존 DB에 game `inserted=41, updated=0, duplicate=0`, meme `inserted=55, updated=0, duplicate=0`, culture `inserted=48, updated=0, duplicate=0`을 순서대로 적용했다. 최종 `documents=153, chunks=296`, 고정 6종 probe `6/6 (100%)`이며 상한 153건 안이다. 다음은 memory ON·knowledge ON 스택 `/health` 경로 대조다.

- 2026-08-27 22:47 KST **실제 대화 스택 기동 intent.** 새 외부 session root를 만들고 memory/knowledge ON 로컬 스택을 기동한다. `KnowledgeDbPath`는 기존 적재 DB로 고정하고 `/health` knowledge documents=153 일치 전에는 직접 대화를 시작하지 않는다. 채점 전 비채택·다음 회차 적용 선언, 사용자 직접 채점, 측정 항목 4종을 유지한다.

- 2026-08-27 19:xx KST **지식 1차 적재 완료 — 회수율 6/6 (100%)**. 상세 = `진행중/AIRI-KNOWLEDGE-INGEST-CONTRACT-2026-08-27.md` §8-3. 서브에이전트 3종 병렬로 범위별 배치 생성 후 lint→ingest→probe. **`documents=0` → `documents=144, chunks=205`** (게임 41 · 밈/슬랭/방송문화 55 · 생활문화 48). 레코드당 청크 2개 이하라 `top_k=4`·`max_chars=1600` 예산에서 서로 경쟁하지 않는다. **아침에 실패했던 고정 질의 6종이 전부 회수된다** — 암베사(폴백"고마워."→암베사 메다르다) · 매도 월드컵(되물음→이상형 월드컵 파생형) · 버니가든2(후원 템플릿 오발동→버니가든2) · 머함(챗봇식 되물음→음슴체 설명) · 둥하(4연속 같은 패턴→**커뮤니티별 고유 인사말 대응 원칙**) · ㄱㄴㅇ(에코 반문→초성체 문화). 둥하가 "흉내 내라"가 아니라 "남의 커뮤니티 것임을 알고 대응하라"로 만들어진 것이 §10-1 판단과 정합. **사실 정정 사례**: 원 채팅 "암베사 팔다리 짤렸어??"는 사실과 다르고(아케인 시즌2 9화 결투 치명상 사망), 이제 **시청자의 틀린 전제를 정정할 근거**가 생겼다 — 폴백도 맞장구도 아닌 세 번째 선택지이며 실측 실패 유형 "지어냄 8건"이 겨냥하던 지점이다. 생성 에이전트가 규칙 6(불확실하면 만들지 않음)으로 제외한 것: LoL 패치 메타 티어·오버워치2 현재 시즌·TFT 세트 번호·실시간 박스오피스/환율·편의점 신상품명(2주 단위 변동 또는 출처 상충). **lint 결함 1건 수리**: `aliases must be unique`가 `casefold()` 기준인데 lint는 개수만 봐서 대소문자 변형 3건이 ingest에서야 거부됐다 — 어느 alias가 겹쳤는지 이름까지 짚도록 보강(테스트 2건). **절차 사고 1건(§8-3-2)**: 생성 에이전트 보고 전에 산출 파일을 적재했고, 그 뒤 파일이 계속 채워졌다 — **레코드 수는 41/55/48 로 내내 같은데 본문만 바뀌었다**(밈 중앙 450→747자, 게임 666→742자, 청크 205→287). "밈 배치가 권장 하한 미만" 이라던 첫 관찰은 미완성본을 본 것이고 최종본은 권장 범위 안이다. alias 중복 수리도 덮어써졌다. **개수·유효건수·회수율이 전부 정상으로 보여 알 수 없었다.** 최종본 재적재로 `updated=30/54`·`duplicate=11/48` 갱신, 회수율 6/6 유지(`--min-hit-rate 0.9` 통과), `documents=145 chunks=289`. 교훈 = **서브에이전트 산출물은 완료 신호를 받은 뒤 만진다**(파일 존재 ≠ 생성 완료). lint 에 최근 수정 경고를 넣었으나 막지는 않는다 — 방금 쓴 배치를 바로 검증하는 것도 정상 사용이고 진짜 방지책은 절차다. 부수 관찰: 생성 에이전트가 세션 scratchpad 의 이전 분석 파일(실제 채팅 토큰 포함)을 발견해 근거로 썼다 — **서브에이전트에는 작업 디렉터리를 격리해 주는 편이 맞다.**

- 2026-08-27 18:xx KST **점수제 픽업 이식 — 개선 없음, 픽업 트랙 종료**. 근거 = `진행중/AIRI-EVAL-INPUT-CONTEXT-AUDIT-2026-08-27.md` §4-4·§11. `broadcast_sim` 에 `chat_replay` 의 점수 체계를 기본 OFF greybox(`fixture.pickup_policy="scored"`)로 이식했다. 가중치는 원본 그대로 옮겼다 — 정답지 57건은 하한이라 맞추면 과적합. **결과: 최고 8.8%(5건) vs 현행 7.0%(4건). 1건 차이는 의미 없다.** 그리고 **모든 점수제 변형이 질문 100%** 다(현행 97%). 산수로 설명된다 — 초당 2.8건이라 5초 창에 14건·질문 1.5건인데 질문 80점 대 리액션 최대 21점(3.8배)이라 질문이 하나만 있어도 끝난다. **실측된 스트리머의 질문 선호는 1.7배**라 가중치 격차가 실제의 두 배 넘게 과하다. 1.7배로 맞추면 분포는 맞지만 그게 곧 피팅이라 중단했다. **중간 오류 1건 기록**: 첫 이식은 선택 규칙만 바꾸고 백로그를 후보로 그대로 둬서 7.0%→7.0% 로 완전 동일했다(질문 비율은 97%→100% 악화). 백로그에 질문이 늘 남아 80점이 항상 이기기 때문 — `pickup_candidate_seconds` 로 후보 창을 실제로 좁힌 뒤에야 변화가 생겼다. **결론: 픽업은 병목이 아니다.** 구조를 통째로 바꿔 얻은 게 1건이고, 완벽히 고쳐도 목표가 "남의 커뮤니티 인사에 잘 답하기" 가 된다. 점수제는 기본 OFF 로 남긴다(입력이 바뀌면 다시 측정 가능). 테스트 6건 신규(기본 경로 바이트 불변·미지 정책 거부·무응답 가능·후보 창 효과 포함), broadcast_sim 105 passed. **권고 순서 확정(§11)**: ①지식 적재 ②**계약 §1(b) 진짜 대화 세션 50턴+** ③그 위에서 게이트 재정의. (c) 리플레이는 (a)(b) 부재로 넣은 대체재이고 구조적으로 맥락을 못 만든다 — 재구성보다 (b) 복귀가 싸고 정확하다.

- 2026-08-27 17:xx KST **M7 절대 판정 근거 상실 · 코덱스 인계 (push 승인 실행)**. 단일 진입점 = `진행중/AIRI-CODEX-HANDOFF-2026-08-27.md`, 근거 = `진행중/AIRI-EVAL-INPUT-CONTEXT-AUDIT-2026-08-27.md` §10. **3축 ≥ 3.0 절대 판정이 현재 평가 입력에서 도달 불가능함을 계산으로 확정**: 사용자 직접 채점 198턴의 `context_retention` 평균 1.54·4점 이상 0건 → 3.0 을 넘으려면 나머지 두 축 평균 3.73(실측 1.98의 1.9배), 맥락 유지가 관측 상한 3.0 까지 올라도 나머지 3.00 필요. 원인 두 겹 = ①스레드 부재(99턴에 시청자 96명·재발화 3%·답 이어받기 3%) ②**채팅이 AIRI 에게 온 것이 아님** — 인사 의식·내부 농담은 다른 스트리머와 그 커뮤니티의 관계 산물이고 픽업의 replay-04 12/99·06b-r3 10/99 를 차지(초반 2·3·4·5 연속). 사용자 지적: *"내가 방송인이었으면 무시했다, 오타인 줄 알았을 것 같다"*. → **S3 `[x]`→`[~]` 강등(측정 불가)**, S2 `[F]`·기준선 1.7576·M7 S0~S4 전체가 같은 입력 위. **회차 간 비교는 유효**(구조적 감점 상수 상쇄), 무효는 절대 판정 하나. **S3-r2 사람 재채점 30턴 취소**(사용자 결정) — 더 정밀하게 재는 것은 "못 넘는다"를 더 정확히 재는 일일 뿐. ROADMAP-STATUS 상단 경고 배너·S3 항목 강등, NEXT-SESSION 배너, WORKING-STATE 갱신으로 코덱스 진입점 전부에 반영했다. 하지 말 것 4건을 인계문 §6 에 명시(S3 를 채택 근거로 쌓지 말 것 / 3.0 절대 판정으로 새 회차 채택 금지 / `pickup_priority` 순서 조정 재시도 금지 / 대화 원문 저장소 반입 금지).

- 2026-08-27 16:xx KST **스트리머 응답 정답지 확보 + 픽업 처방 기각**. SSoT = `진행중/AIRI-EVAL-INPUT-CONTEXT-AUDIT-2026-08-27.md` §4-3·§8. 사용자 지시로 공개 VOD 1편(치지직 `talk` 97분, 여성 버튜버·팔로워 25만, 2026-08-15 지정 관찰 대상 중 1인)을 직접 수집했다 — 채팅 11,974건→가명화 **5,243건·고유 화자 356명**(기존 replay 1,500건의 3.5배), 오디오 90MB, CPU STT **500세그먼트·7,718자**(large-v3-turbo/int8, 2.94배속). 짝짓기로 **스트리머가 실제로 반응한 채팅 57건**을 정답지로 만들었다(에코 364건 제외). **핵심 결과: 오전에 세운 처방이 실측으로 기각됐다.** ①스트리머가 반응한 57건 중 **80.7%가 question 이 아니다**(질문 선호는 1.7배뿐). 현행 정책은 픽업 77건 중 75건을 질문으로 채운다. ②정책 변형 4종 적중률 = 현행 7.0% / **reaction 승격 0.0%** / cooldown 10s 5.3% / stale 300s 3.5% — **제안했던 reaction 승격이 최악**이다. 원인은 `choose_pickup` 이 **엄격한 순위+선착순**이라 리액션을 올리면 백로그 맨 뒤 낡은 채팅부터 읽기 때문. **순위의 순서가 아니라 순위+선착순 구조가 문제**이므로 설정 노브로는 닫히지 않는다. ③답은 레포 안에 있다 — `chat_replay` 의 `offline_fixed_5s_response_sampler_v1` 은 점수제(donation 100·question 80·신호 가점 + novelty/overlap 보정, 20점 미만 no_reply)라 질문 선호를 절대화하지 않고 "반응할 게 없으면 안 한다"가 가능하다. `broadcast_sim` 에만 이 메커니즘이 없다. **도구화 3종**: `import_public_chat.py chzzk-audio`(yt-dlp 가 최신 2026.08.19 에서도 `KeyError('sourceURL')` 로 깨져 재생 API→`otherAttributes.m3u`→CMAF 단일 m4a 로 우회, 서명 토큰을 세그먼트 파일 URL 로 옮기는 계약을 테스트 고정), `stt/transcribe_vod.py`(모델·beam 을 `openai_stt_server.py` 와 핀하는 테스트 포함, GPU 불필요), `pair_streamer_response.py`(어휘 겹침 + **방향 필터** — 접두 매칭으로 조사 변형까지 잡아 후보 70→57건 정제). 테스트 신규 21건, CI 등록 완료. **한계 명시**: 적중률 7%는 절대 성능이 아니다 — 정답지 57건은 하한이고("응 맞아" 류 미탐지) 정책 간 상대 비교로만 유효하다. **범위**: 오디오는 계약 §1 문구에 없으므로 사용자 결정으로 **로컬 평가 한정**(맥락 주입·채점 기준점·픽업 정답지), **학습 정답 사용 금지**. 산출물 전부 저장소 밖.

- 2026-08-27 14:0x KST **지식 대량 적재 트랙 개설 (`fc7705a`)**. 계약서 = `진행중/AIRI-KNOWLEDGE-INGEST-CONTRACT-2026-08-27.md`. **발단: 지식 계층이 내내 비어 있었다** — replay 세 회차(기준선 run 04 포함) 전부 `enabled=True ready=True documents=0 chunks=0 retrievals=0`. 사용자 코멘트가 "지식 결함"으로 지목한 턴들(암베사 폴백·매도 월드컵 되물음·게임을 영화로 지어냄)은 모델 역량이 아니라 **찾아볼 데가 없었던 것**이고, 파인튜닝으로 닫히지 않는다. 사용자 결정(2026-08-27): 사전 대량 적재 + 실시간 검색 계층화, 생성은 클라우드 LLM, 범위는 게임·콘텐츠 / 밈·슬랭·방송문화 / AIRI 자기 설정 / 시사·일반상식 4개 전부. **지연 안전성은 코드로 보장됨** — `retrieve()` 가 턴당 `top_k=4`·`max_chars=1600` 로 제한하고 히트한 턴에만 붙으므로 적재량은 지연이 아니라 디스크·회수품질 문제다. 대신 회수율을 정하는 건 ①한 레코드 한 주제 700~3,000자(24,000자면 40청크로 쪼개져 top_k=4 예산에서 서로 경쟁) ②시청자가 실제로 치는 표기를 담은 `aliases`(BM25 title 가중 4.0). 산출물: `knowledge_batch.py`(lint=배치 상한·배치 간 title 중복·alias 품질·청크 추정 + **ingest 가 거부할 경로 사전 차단**, probe=고정 질의 회수율 측정 `--min-hit-rate` 미달 시 exit 1), 테스트 15, CI `ollama-proxy-memory` 샤드 등록, 고정 질의 세트 `eval/knowledge_probe_queries.txt`(전부 실측 실패 지점). **함정 기록**: `runtime_path()` 가 상대경로를 CWD 기준으로 풀어 runtime-dir 밖으로 나가면 ingest 가 `{"error":"KnowledgeInputError"}` 만 내고 거부한다 — 오늘 실제로 걸렸고 lint 가 잡도록 코드화했다. **end-to-end 실측**: 2건 배치 lint → ingest(`inserted: 2`) → probe 에서 질의 `암베사` 가 alias 경유로 문서 `암베사 메다르다` 회수, 미적재 5건은 정상 미스(1/6). 전체 스위트 2386 passed / 16 skipped / 0 failed. 병행 진행 중인 평가 문항 재설계(사용자 결정: 새 문항 세트 작성)는 계약 §1 "합성 fixture 는 입력이 아니다" 조항과 충돌하므로 개정안에 포함해야 한다 — 미착수.

- 2026-08-27 13:1x KST **채점 출처 정정 + 코멘트 분석 (CRITICAL — 후속 판단 전 반드시 읽을 것)**. 상세 SSoT = `진행중/AIRI-RATING-PROVENANCE-AND-COMMENT-ANALYSIS-2026-08-27.md`. ① **s2-r2·s3-r2는 사람 전수 채점이 아니라 AI 채점 99턴 + 사용자 승인이다**(2026-08-27 사용자 확인). 기준선 run 04와 06b-r3는 사용자가 시트에서 직접 채점한 것이 맞다(다운로드 원본 바이트 크기 일치, AI 초안과 5축 완전일치 6.1%/0.0%). 즉 **기준선과 후보가 서로 다른 자로 측정됐다.** 기존 기록의 "99/99턴 사람 채점"은 s2/s3에 한해 그렇게 읽어야 한다. ② 실측: 앵커 198턴 캘리브레이션 + 시청자 페르소나를 준 AI 채점자 2종이 s3-r2를 1.9798/2.1145로 매겨 승인값 3.0976과 **1.0 차이**, 단 두 AI는 서로 0.1347 안에서 일치 — AI 채점은 프롬프트·프라이밍으로 게이트를 통째로 건너뛴다. 사람 채점 회차에서 잰 AI−사람 편차도 회차마다 부호가 뒤집힌다(+0.15/−0.31). ③ **S3의 PASS는 지지되지 않는다**: 기준선 여유 +0.0976인데 표본 표준오차 ±0.0885(95% 구간 [2.9241, 3.2712]이 3.0을 가로지름)에 자 차이 ±0.3이 더 실린다. 다른 3회차는 −7.59σ~−14.53σ로 확고하고 s3만 +1.10σ다. 사용자가 s3-r2 직접 채점으로 눈금 확정 착수(층화 30턴 중 2턴 완료, 관측 오프셋 +0.1667·잔차 SD≈0.7 → 총 22턴이면 확정, 오프셋 0 근처면 전수로도 판정 보류). ④ **코멘트 분석 198턴**: 사용자가 직접 채점한 두 회차는 99/99턴 전부에 진단 코멘트가 있는데 `summarize_ratings.py`가 설계상 전부 버려 한 번도 집계된 적 없다. 범주 분포 = 템플릿 오발동 35(17.7%, 평균 3축 1.41)·폴백 28(14.1%, 1.10)·안전 사고 25(12.6%, 1.41)·에코 반문 20(10.1%, 2.30)·**정답 모드 13(6.6%, 3.62)**·지어냄 8·**출력 버그 8**·**지식 결함 7**·반복 감지 실패 7·**시청자 이탈 지목 4**·문항 무효 2·미분류 67(33.8%). 함의 = 점수가 같아도(1.10~1.41) 요구되는 수리가 전부 다르다 / **지식 결함 7 + 템플릿 오발동 35 = 42건(21.2%)은 P3 학습 트랙이 손댈 수 없는 영역** / 사용자가 지목한 실제 정답 13건이 합성 학습 데이터 대신 쓰이지 않고 유휴 / 가명 노출·문장 절단·오기 등 버그 8건이 코멘트에만 있고 추적되지 않음 / 계약 §1의 "원 방송 맥락 의존 구분"은 구분 뒤 규정이 없다. 제안 5건은 문서 §3(미승인).

- 2026-08-27 11:33 KST 클로드 PC 재동기화 배치 (로컬 전용, push 미승인). 2026-08-21 배치 7커밋이 push되지 않은 채 원격이 148커밋 앞서 발산해 있었다. `origin/main` 위로 리베이스했고(백업 `backup/claude-2026-08-21-batch-eval` + 태그 `backup-pre-rebase-2026-08-27`), 충돌 5건은 상류 최신본을 기준으로 우리 수리만 얹어 해소했다. `889d4c2`(input_safety 재핀)는 해소 결과 차이가 사라져 리베이스가 자동 소멸시켰다(7커밋). **상류 실측 결함 2건 확인**: ① `origin/main` 자체에서 `test_airi_ko_input_safety_eval` 5건이 `policy hash mismatch`로 실패한다 — 실제 `input_screening_policy_ko.json` 해시는 clean checkout(LF, CR 0바이트)에서 `ba2d49bd…`인데 핀은 `64a7d9bd…`로, CRLF 스머지 작업본에서 핀을 뜬 결함의 재발이다. 별도 워크트리로 상류 단독 재현을 확인했고 08-21 값으로 복원했다(`c5e471a`). ② `3895921`의 시드 하한 강제(`SEEDS=(7,8,9,10)`)와 08-21의 provenance 단언(`[7]`)이 텍스트 충돌 없이 자동 병합돼 `test_compare_broadcast_t3` 2건이 깨졌다 — 단언을 `sorted(SEEDS)`로 픽스처 정의에 결속했다(`609336f`). 전체 스위트 **2354 passed / 16 skipped / 0 failed**. S2 `[F]`·S3 `[x]`·S4 `[P]`와 `goal_status=paused`는 보존한다. 인계문이 지정한 유일한 다음 변수(S4 대표 r2 사람 JSON)는 클로드 PC에 `D:` 드라이브가 없어 원래 확인 불가였으나, 사용자가 `airi-human-eval` 외부 root 전체를 로컬로 제공해 해소됐다 — 실측 결과 **S4 사람 JSON은 아직 없다**(S2-r2·S3-r2 두 건만 존재). S4 r1/r2/r3의 report SHA·exit 0·99턴은 인계문 기록과 전부 일치함을 독립 대조했다.
...
- 2026-08-26 23:31 KST M7-4 S2 native-path receipt. `/api/chat` native stream/non-stream에도 동일한 default-off S2 규칙을 연결하고 seed 11→12·temperature 0.9·호출 2회 회귀를 추가했다. 전체 proxy unittest 407개와 broadcast contract 25개, py_compile, `git diff --check`, roadmap/dashboard·work-continuity 계약이 모두 PASS했다. replay/GPU PID 0, Ollama 11434만 남았고, S2 r2 사람 채점·평가 채택·운영 반영·commit/push는 대기한다.
- 2026-08-26 23:26 KST M7-4 S2 final verification receipt. 경계 보정 테스트를 포함한 proxy unittest 406개와 broadcast contract 25개, py_compile, `git diff --check`, roadmap/dashboard·work-continuity 계약이 모두 PASS했다. replay/GPU PID 0, Ollama 11434만 남았고, S2 r2 사람 채점·평가 채택·운영 반영·commit/push는 대기한다.
- 2026-08-26 23:24 KST M7-4 S2 replay receipt. `AIRI_OPENER_RESAMPLE=on`으로 고정된 07 입력·스트리머 전사·모델 digest·NumCtx 4096·45초 window를 유지해 새 외부 경로 `D:\AIRI-Models\airi-human-eval\20260826-replay-07-s2-r1..r3`에서 순차 실행했다. 세 회차 모두 99턴·exit 0·empty 0·service_error 0·invented_handle 0·polite_violation 0이며 `?` 종결 자동 시그니처는 r1/r2/r3 = 21/65/66, 중앙값 대표는 r2다. fallback은 3/2/3, topic_anchored는 46/18/20, 고유 응답은 96/55/74였고 health-after `opener_resample.enabled=true`, `retry_count=0`을 확인했다. 각 회차 `report.json`, `health-before/after.json`, `review.jsonl`, `rating-sheet.html`, `runner.exit-code.txt`를 검증했으며 report SHA는 r1 `96EE05B730C6E54D8684EAD814D4148A68B1E324D9AD1AA58AF21443BADE9A04`, r2 `CEF34F9AD91695E9B1866427BB5EFA8432D98F78F41141D5062FD13BD9E1B2CD`, r3 `FA7EA724808A30BFDB6A1EA3FF78BE9FF578F4477AFE515892FACA14AA9D6924`다. `AIRI_OPENER_RESAMPLE`은 실행 후 제거됐고 replay/GPU PID 0, Ollama 11434만 남았다. 자동 지표만으로 채택하지 않으며 r2 사람 채점·운영 반영·GPU 학습·commit·push는 대기한다.
- 2026-08-26 23:22 KST M7-4 S2 경계 보정 receipt. S2 재샘플 결과가 언어 게이트에 걸릴 때 기존 비스트림 언어 보정이 추가로 실행될 수 있던 경계를 발견해, `opener_resample_used`가 참이면 추가 언어 retry를 막도록 1줄을 수정했다. S2 계약의 최대 1회 재샘플을 유지하며 proxy unittest 406개를 다시 통과했다.
- 2026-08-26 22:46 KST M7-4 S2 구현·오프라인 receipt. 기본 OFF `AIRI_OPENER_RESAMPLE` gate 아래 첫 요청의 명시적 seed와 직전 assistant/초안의 동일한 첫 2어절·양쪽 `?` 종결을 검증해 seed+1·temperature 0.9로 정확히 한 번 재샘플한다. 프롬프트·grounding 입력은 바꾸지 않으며, stream/non-stream 공통 공개 경로와 health `retry_count`를 추가했다. `python -m unittest test_opener_resample.py test_ollama_proxy.py` 406개와 `test_broadcast_contract.py` 25개, py_compile, roadmap/dashboard·work-continuity·diff-check를 PASS했다. S2 replay·사람 채점·운영 flag·GPU·commit/push는 대기한다.
- 2026-08-26 22:41 KST M7-5 S3 gate verification receipt. 사람 채점 요약과 동결 조건을 재대조한 뒤 `python -m unittest test_broadcast_examples.py test_ollama_proxy.py` 401개와 `python -m unittest test_broadcast_contract.py` 25개를 모두 통과했다. roadmap/dashboard·work-continuity 계약과 `git diff --check`도 PASS했다. S3 평가 채택은 유지하며 운영 flag·GPU 학습·다음 1변수·commit/push는 별도 승인 대기다.
- 2026-08-26 22:38 KST M7-5 S3 사람 채점·평가 게이트 receipt. 외부 `airi-human-rating.json`(SHA256 `CD6E4AD082841C16D4FFDC43C801C1FCB1EDB75A464F1659AD6DD112ED13EAAA`)을 `summarize_ratings.py --baseline`으로 검증했다. 99/99턴·평가자 1명이며 방송다움 2.9798·맥락 3.4040·반응 2.9091·말투 4.9697·사실성 4.8283, 3축 합성 `1.7576 → 3.0976`(+1.34), 5축 평균 delta +1.4949, critical 0, filler 14.14%, invented_name 0, polite 0, 5축≥4 비율 30.3%였다. 후보 채택 조건과 타계책 돌파 정의를 모두 통과해 M7-5를 `[x]`로 올렸고, `AIRI_BROADCAST_EXAMPLES` 운영 기본값 OFF·GPU 학습 금지·commit/push 별도 승인은 유지한다. 다음은 S2 또는 S4 중 한 변수 선택이다.
- 2026-08-26 22:27 KST M7-5 사용자 입력 대기 receipt. 대표 r2 `rating-sheet.html`은 외부 경로에 생성됐지만 `ratings-human.json`이 r2 폴더와 Downloads 어디에도 없어 사람 채점을 시작할 수 없다. 실제 사람 채점이 유일한 채택 게이트이며 브라우저 제어 연결도 없으므로 자동 지표·Codex 추정으로 대체하지 않는다. S3 채택 판정·후속 문서 확정·commit·push는 사용자 채점 JSON이 생길 때까지 중단한다. goal은 blocked로 전환했고, local/origin HEAD `2e98737`, replay/GPU PID 0, Ollama 11434만 남았다.
- 2026-08-26 22:21 KST M7-5 S3 replay receipt. `AIRI_BROADCAST_EXAMPLES=on`으로 기존 07 입력·스트리머 전사·모델 digest·NumCtx 4096·45초 window를 고정해 새 외부 경로 `D:\AIRI-Models\airi-human-eval\20260826-replay-07-s3-r1..r3`에서 순차 실행했다. 세 회차 모두 99턴·exit 0·service_error 0·invented_handle 0·polite_violation 0이며 topic_anchored 27/37/36, fallback 3/6/6, 고유 응답 92/96/96이었다. `?` 종결 자동 시그니처는 45/51/60으로 사전 등록 중앙값 규칙의 대표 회차는 r2다. 각 회차 `report.json`, `health-before/after.json`, `review.jsonl`, `rating-sheet.html`, `runner.exit-code.txt`를 생성·검증했고 report SHA는 r1 `EA3A4AFE5854F19ADC2DE80CA26B002E53A32EBD576CBAC3065F56B41703715E`, r2 `44C28F394561855081091F0424431CDC0F3AB320C9FA3B9EE895B1E779D179EE`, r3 `1E5C2ADD3A1B1FEACC79C213DF4E22D5FD4858D4EDADCAC99C04F6E93087626D`이다. 스택은 종료됐고 Ollama 11434만 남았다. 자동 지표만으로 채택하지 않으며 사람 채점·운영 반영·GPU 학습·commit·push는 대기한다.
- 2026-08-26 22:03 KST M7-5 S3 구현·오프라인 receipt. `ollama-proxy/broadcast_examples.py`에 기본 OFF `AIRI_BROADCAST_EXAMPLES` gate와 네 가지 한국어 반말 U/A 예시, style-only/evidence-exclusion marker를 추가했다. `ollama_proxy.py`는 인증된 live-broadcast context에서만 공통 `inject_response_mode`를 통해 예시 note를 넣고, capability authority와 분리하며, native stream/non-stream에서 name을 제거하고 offline replay seam에서 exact-once 위치를 검증한다. `test_broadcast_examples.py`·`test_ollama_proxy.py` 회귀는 gate/off-path 바이트 동일성·spoof 거부·결정론 evidence pool 제외·idempotence·replay parity를 덮는다. 401 proxy unittest와 25 broadcast-contract unittest·py_compile·`git diff --check`·work-continuity/dashboard/patch-manifest/entrypoint/applicability 계약·sender Node 32/32는 PASS. `test-current-checkpoint.ps1`는 내부 durability 래퍼의 출력/exit 수집이 불안정해 composite PASS로 승격하지 않았다(중복 실행 1회는 ambient runner preflight에서 거부); AIRI replay/GPU·운영 flag·commit·push 없음. M7-5는 `[P]`이며 07 구성 3회 replay와 사람 채점이 남았다.
...
- 2026-08-26 19:10 KST M7-9·M7-10 실행(사용자 "1 2 3 추천 방향", push a642150 완료). M7-10 가드 `f48b1ed`. S6: VOD 오디오(fMP4 접두부 8병렬
  Range·103MB) → STT 618seg(70초) → `--replay-transcript`(`5a5e225`) → run 07 3회 전 복제 무붕괴(최빈 8/3/3, 고유 77/93/96), 화면 맥락
  정답 최초, 가드 실전 발동(한계 기록). 참고 채점 07-r2 3축 2.37(+0.62)·filler 40%·치명 0. 업스트림 v0.12.0-beta.1 해당 없음.
  남은 결정: 07-r2 사람 채점, push.
- 2026-08-26 17:54 KST M7-3 사람 채점 확정(06b-r3, 99턴): 3축 1.90(기준선 1.76, +0.14), filler 55.6%(+10.1%p), 치명 1(23턴 루머 질문 "응" 시인).
  채택 기준 미달 — S1 단독 기각, 운영 flag off 유지(결정 19). 참고 채점과 방송다움 절대차 0.84. M7-10 민감 루머 "응" 시인 가드 후보 추가(결정 21).
  다음: S6 입력 정합 방식 사용자 결정(M7-9).
- 2026-08-26 17:32 KST M7 S1 정정. 문형 무관 지표(최빈 응답 빈도)로 재검토: 되먹임 3채널 차단은 템플릿 고정만 없애고 문구 고정은 2/3에 남음
  (06b-r3 "응, 지금 채팅 보고 있어" 39턴 = 비트 큐 되풀이; off-r2도 한 문장 78턴). Claude 참고 채점 06b-r3 3축 1.59(기준선 1.76) —
  개선 없음, 실패 모드 교체. 원인은 맥락 빈곤 입력(원 방송 화면 전제 채팅)+모델 기본 응답이며 되먹임은 증폭기. 운영 flag 요청 없음.
  M7-9 S6 입력 정합(자기완결 구간 선별 vs 스트리머 STT 발화 정렬 공급) 추가, 사용자 결정 대기.
- 2026-08-26 17:25 KST M7-1·M7-2 실행(사용자 "1 2 3 4 모두 진행"). push `7a998bb..e6e745a`. S0: run 05 구성 3회 복제 — 픽업 결정론, 생성 비재현,
  "?" 종결 78/71/15/84·"음" 48/56/1/89(SD ≈30, n=1 무효). S1: `098c887` `AIRI_FEEDBACK_HYGIENE`(기본 off byte 동일) + `f748cae` 3축 합성·`--baseline`;
  복제 3회씩 on 붕괴 1/3, journal 2/3, on+브리핑 자기 에코 차단 0/3(off 3/4) — 되먹임 고리가 붕괴 원인. 운영 디렉터에는 에코 채널 없음. 채점
  시트 06b-r3(사전 등록 중앙값)·06a-r1·run 05 준비. `진행중/AIRI-M7-S0-S1-RECEIPT-2026-08-26.md`. 남은 결정: 사람 채점, push, 운영 flag on.
- 2026-08-26 16:55 KST 사용자 지시로 타계책을 문서에 구조적으로 반영했다. ROADMAP M7 신설(M7-1 S0 복제 측정 `[D]`, M7-2 S1 proxy 되먹임
  flag `[D]`, M7-3 run 05 사람 채점 `[D]`, M7-4~7 S2~S5 `[ ]`, M7-8 파인튜닝 재진입 `[B]` 2026-09-09까지), M6 이력화, §0-1 원칙 5~7,
  §1 사람 채점 주 지표 표(3축 합성 1.76→≥3.0·critical 0·filler ≤25%·복제 ≥2), P3-M2→S5 연결, 파인튜닝 행 재개 조건 강화, 결정 큐
  17~18. 평가 계약 §4 돌파 정의·§5 복제·§6 시그니처 표시 전용, 실태 문서 §4.2 재개 조건 강화. 실행은 사용자 승인 뒤.

## 2026-08-26 M6 — 실제 대화 사람 채점 체계·배관 수리 (사용자 4건 승인)

- 2026-08-26 16:35 KST M6-10 타계책. 사용자 goal("코드 vs 파인튜닝 갈팡질팡, 돌파 못 함")에 ultracode 워크플로(렌즈 4·반박 8)로 답했다.
  실제 병목: (1) 런타임 자기 되먹임 고리 — run 04→05 "음" 오프너 1→48·"?" 종결 20→78·재료 사용 35→0(같은 모델·입력·샘플링),
  proxy `_short_direct_answer`·journal assistant 회수가 운영 경로에도 같은 고리; (2) 메모형 재료 수용 0/12; (3) n=1 측정. 계획
  S0 복제 측정 → S1 proxy 되먹임 차단(flag) → S2 오프너 재샘플 → S3 예시 4쌍 → S4 픽업 스킵 → S5 qwen3:8b 1회 진단(채택 아님);
  돌파 정의 3축 합성 ≥3.0·critical 0·filler ≤25%; 파인튜닝 2주 금지, 재진입은 실제 채팅+사람 검증 ≥300턴. `진행중/AIRI-BREAKTHROUGH-PLAN-2026-08-26.md`.
- 2026-08-26 15:35 KST M6-8·M6-9. 사용자가 추천안 A+B를 채택해 `beb0559`로 구현(P5 축자 인용 제거·안전 화면·재생 후원 티어·
  의례 history 되먹임 차단·한국어식 가명). run 05(같은 입력): 치명 0, 축자 인용 0, "고마워." 0으로 후원 경로는 정상화됐지만
  모델이 "음... 뭐가 X인데?" 되묻기 문형에 갇힘(37턴, raw 모델 출력). Claude 참고 채점 1.70/1.65/1.81/4.22/3.94, filler 81%.
  남은 결함은 결정론 축이 아닌 모델·프롬프트 축 — M6-10 후보 D(되먹임 위생 일반화)/E(qwen3:8b 평가 전용 probe)/F(프롬프트
  규칙)를 사용자 결정에 올림. `진행중/AIRI-REAL-CHAT-REPLAY-RUN05-2026-08-26.md`, D1 contract §11.
- 2026-08-26 14:15 KST M6-7 종료 — 사람 채점 기준선. 사용자가 run 04 99턴을 채점: 방송다움 1.87·맥락 1.58·반응 1.83·말투
  3.60·사실성 2.75, 치명 7.1%, 무의미 대꾸 45.5%, 5축≥4 0%. Claude 참고 채점과 앞 3축 평균 절대차 ≤0.51. 코멘트
  우선순위: 후원 의례 오발동·축자 인용·안전 회피(A), "고마워." 붕괴(B), 에코 반문·챗봇 말투(C). 계약 §4에 기준선
  확정, M6-8은 사용자 결정 대기.
...
- (추기 2026-08-21) continuity governance 확인 종결 — 사용자가 코덱스 세션의 일괄 승인 실재를 직접 확인("맞아 승인했어"). 판정을 "위반"→"승인 기록 누락"으로 하향, E1 유효 후보 유지. 공식 기록 = AIRI-CODEX-BATCH-EVAL-2026-08-21.md §7.
