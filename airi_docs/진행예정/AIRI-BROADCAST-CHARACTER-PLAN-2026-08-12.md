...
| 3 | **캐릭터 확정** — 이름(AIRI 유지 여부)·시그니처 인사·팬덤명 방향 + T-05 한국어 음성 레퍼런스 화자 승인 | **이름 AIRI 유지 확정, 메타 서사 호칭 "주인님" 확정.** 시그니처 인사·팬덤명은 포지 후보 제안 → 사용자 확정 대기(캐릭터 헌법 §6). T-05는 dev PC에서 후보 화자 합성 샘플 생성 → 사용자 청취 검토 1회 → 어울리는 게 없으면 현행(일본어 참조 교차클로닝) 유지 |
