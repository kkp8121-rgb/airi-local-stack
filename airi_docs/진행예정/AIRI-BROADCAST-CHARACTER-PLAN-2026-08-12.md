...
- 상태: **계획 승인 대기 + 사용자 결정 4건 처리 완료(2026-08-12, 결정 2는
  조건부)**. 코드 변경 없음.
...
| # | 결정 | 결과 |
|---|---|---|
| **1** | **관계 축** — 사용자가 방송에 출연하는가 | **AI 단독형 + 메타 서사 채택.** 사용자는 화면 밖 "주인님"으로만 존재. 성능·하드웨어 한계 자학 개그를 콘텐츠화 허용(경계는 캐릭터 헌법 §5 메타 서사 절). 채팅 실질 공동 진행자화·시청자 기억 콜백 1차 축·팬덤명 포지 제안→사용자 확정 등 보완장치 유지 |
...
| 3 | **캐릭터 확정** — 이름(AIRI 유지 여부)·시그니처 인사·팬덤명 방향 + T-05 한국어 음성 레퍼런스 화자 승인 | **이름 AIRI·호칭 "주인님"·시그니처 인사(메타 개그형)·팬덤명 "아이리스"·클로징(메타 개그형) 전부 확정**(확정 문구는 캐릭터 헌법 §6). T-05는 dev PC에서 후보 화자 합성 샘플 생성 → 사용자 청취 검토 1회 → 어울리는 게 없으면 현행(일본어 참조 교차클로닝) 유지 |
