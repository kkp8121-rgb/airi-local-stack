...
$monitorServer = [IO.Path]::GetFullPath((Join-Path $here 'monitor_server.py'))
...
$p = Start-Process -FilePath $python -ArgumentList $monitorServer -WorkingDirectory $here -RedirectStandardOutput $outLog -RedirectStandardError $errLog -WindowStyle Hidden -PassThru
