...
> **2026-08-07 갱신:** 코드 감사 후 결함 수정이 브랜치 `fix/code-audit-remediation-2026-08-07`에 반영됐다. **최신 진입점은 `airi_docs/AIRI-FIX-HANDOFF-2026-08-07.md`** — 그 문서의 §2 절차(테스트 재실행 → 패치 상태 확정 → 스택 기동 → 마이크 5회 acceptance)를 따르라. 패치 세트 구성이 바뀌었으므로(볼륨 폴백 900→2700ms 방식, flush 400ms, transcript-latency deprecated) 아래 본문 중 패치 관련 서술은 구버전 기준이다.
